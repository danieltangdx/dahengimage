...
#else
   #ifdef _WIN64
      #define J_REG_DATABASE  "$(JAI_SDK_BIN_64)\\Registry.xml"   ///< Default location of the JAI SDK Registry database file
   #else
...
#ifdef __BORLANDC__
   // The Borland C++ Builder does not work with the Class callback functions because
   // the JAI SDK is created using Microsoft Visual C/C++ compiler!
   // This means that the callback function has to be a static function!
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_Open(const int8_t* psPrivateData, FACTORY_HANDLE *pHandle);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_Close(FACTORY_HANDLE hFac);
...
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_GetInfo(J_FACTORY_INFO info, int8_t* pBuffer, uint32_t* pSize);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_UpdateCameraList(FACTORY_HANDLE hFac, bool8_t *bHasChanged);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_GetNumOfCameras(FACTORY_HANDLE hFac, uint32_t *pNum);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_GetCameraIDByIndex(FACTORY_HANDLE hFac, int32_t iNum, int8_t* pBuffer, uint32_t* pSize);
...
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_GetCameraInfo(FACTORY_HANDLE hFac, int8_t* pCameraId, J_CAMERA_INFO InfoId, int8_t* pBuffer, uint32_t* pSize);
...
/// \retval                       Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_EnableForceIp(FACTORY_HANDLE hFac, uint32_t ForceEnabled);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_Open(FACTORY_HANDLE hFac, int8_t* pCameraID, CAM_HANDLE* hCam);
...
/// When a cameras has been opened using \c J_Camera_Open() it needs to be closed using \c J_Camera_Close()
/// \sa \c J_Camera_Close(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_OpenMc(FACTORY_HANDLE hFac, int8_t* pCameraID, CAM_HANDLE* hCam, J_DEVICE_ACCESS_FLAGS iOpenFlags, uint32_t iMcIP);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_Close(CAM_HANDLE hCam);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetValueInt64(CAM_HANDLE hCam, int8_t* sNodeName, int64_t *pValue);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetValueDouble(CAM_HANDLE hCam, int8_t* sNodeName, double *pValue);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetValueString(CAM_HANDLE hCam, int8_t* sNodeName, int8_t* ValueStr, uint32_t* pSize);
...
/// The node type has to be \c IInteger, \c IEnumeration or \c IBoolean
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_SetValueInt64(CAM_HANDLE hCam, int8_t* sNodeName, int64_t Value);
...
/// The node type has to be \c IFloat
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_SetValueDouble(CAM_HANDLE hCam, int8_t* sNodeName, double Value);
...
/// The node type has to be \c IFloat
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_SetValueString(CAM_HANDLE hCam, int8_t* sNodeName, int8_t* ValueStr);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_ExecuteCommand(CAM_HANDLE hCam, int8_t* sNodeName);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_ReadData(CAM_HANDLE hCam, int64_t iAddress, void* pData, uint32_t* pSize);
...
/// The camera needs to be opened using \c J_Camera_Open() before read and write operations can be performed
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_WriteData(CAM_HANDLE hCam, int64_t iAddress, const void* pData, uint32_t* pSize);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetConfigInfo(CAM_HANDLE hCam, J_CONFIG_INFO cinfo, int8_t* pBuffer, uint32_t* pSize); 
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetNumOfNodes(CAM_HANDLE hCam, uint32_t* pNum);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetNodeByIndex(CAM_HANDLE hCam, uint32_t index, NODE_HANDLE* phNode);
...
/// The camera needs to be opened using \c J_Camera_Open() before the nodes can be opened
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetNodeByName(CAM_HANDLE hCam, int8_t* sNodeName, NODE_HANDLE * phNode);
...
/// \endcode
///  \sa \c J_Camera_Open(), \c J_Node_GetName(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetNumOfSubFeatures(CAM_HANDLE hCam, int8_t* sParentNodeName, uint32_t *pNum);
...
/// \endcode
///  \sa \c J_Camera_Open(), \c J_Node_GetName(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetSubFeatureByIndex(CAM_HANDLE hCam, int8_t* sParentNodeName, uint32_t index, NODE_HANDLE * phNode);
...
/// \sa \c J_Camera_Open(), \c J_Node_GetName(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetFeatureByName(CAM_HANDLE hCam, int8_t* sFeatureNodeName, NODE_HANDLE* phNode);
...
/// The camera needs to be opened using \c J_Camera_Open() before the feature nodes can be opened
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_InvalidateNodes(CAM_HANDLE hCam);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetNumOfDataStreams(CAM_HANDLE hCam, uint32_t *pNum);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_CreateDataStream(CAM_HANDLE hCam, uint32_t iChannel, STREAM_HANDLE *pDS);
...
/// The camera needs to be opened using \c J_Camera_Open() before the data stream can be created.
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_CreateDataStreamMc(CAM_HANDLE hCam, uint32_t iChannel, STREAM_HANDLE *pDS, DWORD iMcIP=0);
...
/// These flags are currently not used by the drivers!!
...
/// \brief The J_ACQ_STOP_FLAGS are used to control the stop of image acquisition using \c J_DataStream_StopAcquisition()
/// These flags are currently not used by the drivers!!
typedef enum  _J_ACQ_STOP_FLAGS_TYPE
{
   ACQ_STOP_FLAGS_NONE = 0,      ///< The \c J_DataStream_StopAcquisition() will wait until the current image has been received completely before stopping the acquisition.
   ACQ_STOP_FLAG_KILL = 1        ///< Kill ongoing acquisition instead of waiting for the acquisition of the current image to complete.
} J_ACQ_STOP_FLAGS;
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_AnnounceBuffer(STREAM_HANDLE hDS, void *pBuffer, uint32_t uiSize, void *pPrivate, BUF_HANDLE *hBuf);
...
/// \endcode
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_FlushQueue(STREAM_HANDLE hDS, J_ACQ_QUEUE_TYPE QueueType);


//******************************************************************************************************************
/// \brief                 Start the image acquisition on the stream channel
/// \param [in] hDS        Handle to a valid data stream, obtained by \c J_Camera_CreateDataStream() function.
/// \param [in] iFlags     Aquisition start control flag. This parameter is currently not implemented in the GigE Vision drivers so it has currently no effect!!
/// \param [in] iNumImages Number of images to acquire. This parameter is currently not implemented in the drivers so it has currently no effect!!.
/// \retval                Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
...
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_StartAcquisition(STREAM_HANDLE hDS, J_ACQ_START_FLAGS iFlags = ACQ_START_FLAGS_NONE, uint64_t iNumImages = 0);

//******************************************************************************************************************
/// \brief                 Stop the image acquisition on the stream channel
/// \param [in] hDS        Handle to a valid data stream, obtained by \c J_Camera_CreateDataStream() function.
/// \param [in] iFlags     Aquisition stop control flag. This parameter is currently not implemented in the GigE Vision Filter driver but it is necessary to use the ACQ_STOP_FLAGS_KILL if you use the Socket Driver!!
/// \retval                Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_StopAcquisition(STREAM_HANDLE hDS, J_ACQ_STOP_FLAGS iFlags = ACQ_STOP_FLAGS_NONE);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_GetStreamInfo(STREAM_HANDLE hDS, J_STREAM_INFO_CMD iCmd, void *pBuffer, uint32_t *pSize);
...
/// \retval                Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_GetBufferID(STREAM_HANDLE hDS, uint32_t iIndex, BUF_HANDLE *hBuf);
...
/// \endcode
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_RevokeBuffer(STREAM_HANDLE hDS, BUF_HANDLE hBuf, void **pBuffer, void **pPrivate);
...
/// \endcode
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_QueueBuffer(STREAM_HANDLE hDS, BUF_HANDLE hBuf);
...
/// \retval                     Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_STREAM_INFO_CMD
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_GetBufferInfo(STREAM_HANDLE hDS, BUF_HANDLE hBuf, J_BUFFER_INFO_CMD iCmd, void *pBuffer, uint32_t *pSize);
...
/// \code
/// // Example:
/// // If we want to find out if the Packet Resend function is enabled then we need code like this:
/// bool IsResendEnabled(void)
/// {
///    bool bCanResend = false;
///    J_STREAM_PARAM_CMD iCommand = STREAM_PARAM_CMD_CAN_RESEND; 
///    uint32_t iCanResend = 0; // Init to something...
///    uint32_t iSize = sizeof(iCanResend); // Here we need to set the size of the parameter variable we are reading
///    
///    // Be aware that you need to pass in the handle of the DataStream and NOT the handle to the camera object!!
///    retval = J_DataStream_GetParam(m_hDC, iCommand, (void*)&iCanResend , &iSize);
///    if (retval != J_ST_SUCCESS) 
///    {
///       ShowErrorMsg(CString("Could not get STREAM_PARAM_CMD_CAN_RESEND!"), retval); 
///    }
///    
///    // Here the iCanResend will be 0 if the resend is disabled and <>0 if resend is enabled 
///    if (iCanResend)
///       bCanResend = true;
///    else
///       bCanResend = false;
...
/// \endcode
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_DataStream_GetParam(STREAM_HANDLE hDS, J_STREAM_PARAM_CMD iCmd, void *pBuffer, uint32_t *pSize);
...
/// // Example:
/// // If we want to find out if the Packet Resend function is enabled then we need code like this:
/// vois SetResendEnabled(bool bEnable)
/// {
///    J_STREAM_PARAM_CMD iCommand = STREAM_PARAM_CMD_CAN_RESEND; 
///    uint32_t iCanResend = 0; // Init to something...
///    uint32_t iSize = sizeof(iCanResend); // Here we need to set the size of the parameter variable we are reading
...
///
///    // Be aware that you need to pass in the handle of the DataStream and NOT the handle to the camera object!!
///    retval = J_DataStream_SetParam(m_hDC, iCommand, (void*)&iCanResend , &iSize);
///    if (retval != J_ST_SUCCESS) 
///    {
///       ShowErrorMsg(CString("Could not get STREAM_PARAM_CMD_CAN_RESEND!"), retval); 
///    }
...
/// \endcode
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_DataStream_SetParam(STREAM_HANDLE hDS, J_STREAM_PARAM_CMD iCmd, void *pBuffer, uint32_t *pSize);
...
/// \param [in,out] pSize      The size of the buffer.
///                            The function sets the actual size of data stored into the buffer.
/// \retval                    Status code defined in the J_STATUS_CODES. If the event queue is empty then this function returns J_ST_NO_DATA. If the function succeeds, returns J_ST_SUCCESS.
/// \note
...
/// function will return the image buffer handle as described by the \c J_EVENT_DATA_NEW_IMAGE structure. It is VERY IMPORTANT
/// to note that \c J_Event_GetData() will remove the new image from the delivery queue and the user applications then has
/// to return the image buffer using \c J_DataStream_QueueBuffer()!
...
/// HANDLE hCondition;
/// error = J_Event_CreateCondition(&hCondition);
///
/// if ((error != J_ST_SUCCESS) || (hCondition == NULL))
...
/// error = J_Camera_RegisterEvent(m_hCamera, EVENT_GEV_EVENT_CMD, hCondition, &hEvent);
///
/// while(m_bEnableThread)
/// {
///   error = J_Event_WaitForCondition(hCondition, 1000, &iWaitResult);
///
...
///       {
///         error = J_Event_SignalCondition(hCondition);
///       }
///     }
///   }
...
/// }
///
/// // Un-register the GigE Vision Event command event with the transport layer
/// error = J_Camera_UnRegisterEvent(m_hCamera, EVENT_GEV_EVENT_CMD);
///
/// // Free the event object
/// if (hEvent != NULL)
/// {
///   error = J_Event_Close(hEvent);
///   hEvent = NULL;
/// }
///
/// // Free the connection event object
/// if (hCondition != NULL)
///   error = J_Event_CloseCondition(hCondition);
/// hCondition = NULL;
///
///  m_bThreadRunning = false;
/// \endcode
/// \sa \c J_Event_Register(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_GetData(EVT_HANDLE pEventHandle, void *pBuffer, uint32_t *pSize);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_GetInfo(EVT_HANDLE pEventHandle, J_EVENT_INFO_ID iID, void *pBuffer, uint32_t *pSize);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Camera_RegisterEvent(), \c J_DataStream_RegisterEvent(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_Flush(EVT_HANDLE pEventHandle);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Camera_RegisterEvent(), \c J_DataStream_RegisterEvent(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_Close(EVT_HANDLE pEventHandle);
...
typedef enum _J_EDC_CONNECTION_TYPE
{
   ...
   J_EDC_LOST_CONTROL = 2,     //!< The camera timed out the connection. We cannot control the camera before re-acquiring control again. 
   J_EDC_GOT_CONTROL  = 3,     //!< We regained control of the camera. THIS HAS BEEN DEPRECATED AND WILL NOT BE CREATED BY THE TRANSPORT LAYER
...
/// \endcode
/// \sa \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Camera_RegisterEvent(CAM_HANDLE hCam, J_EVENT_TYPE iEventType, HANDLE hEvent, EVT_HANDLE *pEventHandle);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Camera_UnRegisterEvent(CAM_HANDLE hCam, J_EVENT_TYPE iEventType);
...
/// EVENT_HANDLE   hEvent;
/// 
/// // Register the hCondition with the acquisition engine to be signaled whenever a 
/// // new image buffer is ready.
/// error = J_DataStream_RegisterEvent(m_hDS, EVENT_NEW_BUFFER, hCondition, &hEvent);
///
/// // Start the image acquisition
/// error = J_DataStream_StartAcquisition(m_hDS, ACQ_START_NEXT_IMAGE, 0 );
///
...
///     error = J_Event_GetData(hEvent, &hBuffer,  &iSize);
///
///     if (error == J_ST_SUCCESS)
///     {
///       // Get the pointer to the frame buffer.
///       iSize = (uint32_t)sizeof (void *);
///       error = J_DataStream_GetBufferInfo(m_hDS,
///                                          hBuffer,
///                                          BUFFER_INFO_BASE,
...
///         error = J_Image_ShowImage(reinterpret_cast<VW_HANDLE>(g_hWin),
///                                   &tAqImageInfo,
///                                   m_iRGain,
///                                   m_iGGain,
///                                   m_iBGain);
...
///       error = J_DataStream_QueueBuffer(m_hDS, iBufferID);
///     }
///   }
///   else if(iWaitResult == J_COND_WAIT_EXIT)
///   {
...
/// }
///
/// // Un-register the hCondition
/// error = J_DataStream_UnRegisterEvent(m_hDS, EVENT_NEW_BUFFER);
/// 
/// // Free the event object
/// if (hEvent != NULL)
/// {
...
/// \endcode
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_RegisterEvent(STREAM_HANDLE hDS, J_EVENT_TYPE iEventType, HANDLE hEvent, EVT_HANDLE *pEventHandle);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Camera_CreateDataStream(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_DataStream_UnRegisterEvent(STREAM_HANDLE hDS, J_EVENT_TYPE iEventType);
...
typedef enum _J_COND_WAIT_RESULT_TYPE
{
       ...
       J_COND_WAIT_SIGNAL=1,     //!< \c J_Event_WaitForCondition() got signaled using \c J_Event_SignalCondition() or \c J_Event_WaitForMultipleConditions() got signaled on Condition 0 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT=2,       //!< \c J_Event_WaitForCondition() got signaled using \c J_Event_ExitCondition() or \c J_Event_WaitForMultipleConditions() got signaled on Condition 0 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_1=3,   //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 1 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_1=4,     //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 1 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_2=5,   //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 2 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_2=6,     //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 2 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_3=7,   //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 3 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_3=8,     //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 3 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_4=9,   //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 4 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_4=10,    //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 4 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_5=11,  //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 5 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_5=12,    //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 5 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_6=13,  //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 6 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_6=14,    //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 6 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_7=15,  //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 7 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_7=16,    //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 7 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_8=17,  //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 8 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_8=18,    //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 8 using \c J_Event_ExitCondition()
       J_COND_WAIT_SIGNAL_9=19,  //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 9 using \c J_Event_SignalCondition()
       J_COND_WAIT_EXIT_9=20,    //!< \c J_Event_WaitForMultipleConditions() got signaled on Condition 9 using \c J_Event_ExitCondition()
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_CreateCondition(HANDLE* pConditionHandle);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_SignalCondition(HANDLE conditionHandle);
...
/// \retval                      Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Event_CreateCondition(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_ExitCondition(HANDLE conditionHandle);
...
/// \retval                      Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Event_CreateCondition(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_ResetCondition(HANDLE conditionHandle);
...
///
/// // Register the GigE Vision Event command event with the transport layer
/// iRet = J_Camera_RegisterEvent(m_hCamera, EVENT_GEV_EVENT_CMD, hCondition, &hEvent);
///
/// while(m_bEnableThread)
/// {
///   error = J_Event_WaitForCondition(hCondition, 1000, &iWaitResult);
...
///       if ((error == J_ST_SUCCESS) && (NumEventsInQueue > 0))
///       {
///         J_Event_SignalCondition(hCondition);
///       }
///     }
///   }
///   else if(iWaitResult == J_COND_WAIT_EXIT)
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_WaitForCondition(HANDLE conditionHandle, DWORD timeout, J_COND_WAIT_RESULT* pWaitResult);

//******************************************************************************************************************
/// \brief                       Wait for one of the multiple Conditions to be signaled.
/// \param [in] nCount           Number of condition handles to handle. This have to be between 1 and 10 
/// \param [in] conditionHandles Pointer to array of internal Condition handles obtained by calls to the \c J_Event_CreateCondition() function.
/// \param [in] timeout          Timeout for the Wait operation. If timeout is set to 0 then it will wait forever
/// \param [in] pWaitResult      Result for the Wait operation. This can either be TIMEOUT, SIGNAL, EXIT or ERROR
/// \retval                      Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \code
/// // This small piece of code demonstrate the use of multiple Camera Events simultaneously.
/// uint32_t    iSize;
/// J_EVENT_DATA_GEV_EVENT_CMD   GigEEventData;
...
///
/// J_COND_WAIT_RESULT iWaitResult;
/// HANDLE             ConditionList[2];
/// HANDLE             hGevEventCondition;
/// HANDLE             hConnectionCondition;
///
/// // Create the two conditions to be used
/// error = J_Event_CreateCondition(&hGevEventCondition);
/// if ((error != J_ST_SUCCESS) || (hGevEventCondition == NULL))
///   fprintf(stderr, " J_Event_CreateCondition failed for the GigE Vision Events!\n");
///
/// error = J_Event_CreateCondition(&hConnectionCondition);
/// if ((error != J_ST_SUCCESS) || (hConnectionCondition == NULL))
///   fprintf(stderr, " J_Event_CreateCondition failed for the Camera Connection Events!\n");
///
/// // Set up the Condition List
/// ConditionList[0] = hGevEventCondition;
/// ConditionList[1] = hConnectionCondition;
///
/// EVT_HANDLE  hGevEvent; // Connection event handle
///
/// // Register the GigE Vision Event command event with the transport layer
/// iRet = J_Camera_RegisterEvent(m_hCamera, EVENT_GEV_EVENT_CMD, hGevEventCondition, &hGevEvent);
///
/// EVT_HANDLE  hConnectionEvent; // Connection event handle
///
/// // Register the connection event with the transport layer
/// iRet = J_Camera_RegisterEvent(m_hCamera, EVENT_CONNECTION, hConnectionCondition, &hConnectionEvent); 

/// while(m_bEnableThread)
/// {
///   error = J_Event_WaitForMultipleConditions(2, ConditionList, 1000, &iWaitResult);
///
///   switch (iWaitResult)
///   {
///     case J_COND_WAIT_SIGNAL: // The first Condition (GigE Vision Event) got signalled
///       // Get the GigE Vision Event Data from the event
///       iSize = (uint32_t)sizeof(GigEEventData);
///       error = J_Event_GetData(hGevEvent, &GigEEventData,  &iSize);
///
///       // Here we need to process the EVENT telegram
///       if (error == J_ST_SUCCESS)
///       {
...
///         iSize = sizeof(EventInfo);
///
///         error = J_Event_GetInfo(hGevEvent, (J_EVENT_INFO_ID)EVENT_INFO_NUM_ENTRYS_IN_QUEUE, &NumEventsInQueue, &iSize);
///
///         if ((error == J_ST_SUCCESS) && (NumEventsInQueue > 0))
///         {
///           J_Event_SignalCondition(hGevEventCondition);
///         }
///       }
///       break;
///
///     case J_COND_WAIT_SIGNAL_1: // The second Condition (Camera Connection Event) got signalled
///       // Get the Connection Info from the event
///       iSize = (uint32_t)sizeof(J_EVENT_DATA_CONNECTION);
///       error = J_Event_GetData(hConnectionEvent, &DataConnectionInfo,  &iSize);
///
///       if (error == J_ST_SUCCESS)
///       {
...
///             // Do something with the CONNECTED event
///             ...
///             break;
///           case J_EDC_DISCONNECTED:
///             // Do something with the DISCONNECTED event
///             ...
///             break;
///           case J_EDC_LOST_CONTROL:
///             // Do something with the LOST CONTROL event
///             ...
///             break;
///           case J_EDC_GOT_CONTROL: // Deprecated!!
///             // Do something with the GOT CONTROL event
///             ...
///             break;
///         } // switch(DataConnectionInfo.m_iConnectionState)
///       
...
///         iSize = sizeof(EventInfo);
///
///         error = J_Event_GetInfo(hConnectionEvent, (J_EVENT_INFO_ID)EVENT_INFO_NUM_ENTRYS_IN_QUEUE, &NumEventsInQueue, &iSize);
///
///         if ((error == J_ST_SUCCESS) && (NumEventsInQueue > 0))
///         {
///           J_Event_SignalCondition(hConnectionCondition);
///         }
///       break;
///
...
/// if (hGevEvent != NULL)
/// {
///   error = J_Event_Close(hGevEvent);
///   hGevEvent = NULL;
/// }
///
/// // Free the connection event object
/// if (hGevEventCondition != NULL)
///   error = J_Event_CloseCondition(hGevEventCondition);
/// hGevEventCondition = NULL;
///
/// // Un-register the Camera Connection event with the transport layer
/// error = J_Camera_UnRegisterEvent(m_hCamera, EVENT_CONNECTION);
///
/// // Free the event object
/// if (hConnectionEvent != NULL)
/// {
///   error = J_Event_Close(hConnectionEvent);
///   hConnectionEvent = NULL;
/// }
///
/// // Free the connection event object
/// if (hConnectionCondition != NULL)
///   error = J_Event_CloseCondition(hConnectionCondition);
/// hConnectionCondition = NULL;
///
/// m_bThreadRunning = false;
/// \endcode
/// \sa \c J_Event_CreateCondition(), \c J_Event_SignalCondition(), \c J_Event_ExitCondition(), \c J_Event_ResetCondition(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_WaitForMultipleConditions(uint32_t nCount, const HANDLE* conditionHandles, DWORD timeout, J_COND_WAIT_RESULT* pWaitResult);
...
/// J_COND_WAIT_RESULT iWaitResult;
/// HANDLE           hCondition;
/// error = CreateCondition(&hCondition);
///
/// if ((error != J_ST_SUCCESS) || (hCondition == NULL))
...
/// while(m_bEnableThread)
/// {
///   error = WaitForCondition(hCondition, 1000, &iWaitResult);
///
///   if (J_COND_WAIT_SIGNAL == iWaitResult)
///   {
///     if (m_bEnableThread == false)
...
/// // Free the connection event object
/// if (hCondition != NULL)
///   error = CloseCondition(hCondition);
/// hCondition = NULL;
///
/// m_bThreadRunning = false;
/// \endcode
/// \sa \c J_Event_CreateCondition(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE  __cdecl J_Event_CloseCondition(HANDLE conditionHandle);
...
/// \brief Representation of a Integer and Float GenICam node
typedef enum _J_NODE_REPRESENTATION_TYPE
{
...
} J_NODE_REPRESENTATION;

/// \brief Display Notation for a Float GenICam node
typedef enum _J_NODE_DISPLAY_NOTATION_TYPE
{
...
} J_NODE_DISPLAY_NOTATION;

/// \brief GenICam Error Info from the last error returned from the GenICam library
typedef struct {
   char sDescription[1024]; //!< Error description received from the GenICam library
   char sNodeName[128];     //!< GenICam Node Name for the node where the error was detected
} tGenICamErrorInfo;
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetAccessMode(NODE_HANDLE hNode, J_NODE_ACCESSMODE *pValue);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetName(NODE_HANDLE hNode, int8_t* pBuffer, uint32_t* pSize, uint32_t FullQualified = 0);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetNameSpace(NODE_HANDLE hNode, J_NODE_NAMESPACE *pValue);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetVisibility(NODE_HANDLE hNode, J_NODE_VISIBILITY* pVisibility);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_Invalidate(NODE_HANDLE hNode);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetIsCachable(NODE_HANDLE hNode, uint32_t* pVal);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetCachingMode(NODE_HANDLE hNode, J_NODE_CACHINGMODE *pValue);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetPollingTime(NODE_HANDLE hNode, int64_t *pValue);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetToolTip(NODE_HANDLE hNode, int8_t* pBuffer, uint32_t* pSize);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetDescription(NODE_HANDLE hNode, int8_t* pBuffer, uint32_t* pSize);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetDisplayName(NODE_HANDLE hNode, int8_t* pBuffer, uint32_t* pSize);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetEventID(NODE_HANDLE hNode, int8_t* pBuffer, uint32_t* pSize);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetIsStreamable(NODE_HANDLE hNode, uint32_t* pVal);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetNumOfProperties(NODE_HANDLE hNode, uint32_t* pNum);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetPropertyNameByIndex(NODE_HANDLE hNode, uint32_t index, int8_t* pBuffer, uint32_t* pSize);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetPropertyByName(NODE_HANDLE hNode, const int8_t* PropertyName, 
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_ImposeAccessMode(NODE_HANDLE hNode, J_NODE_ACCESSMODE AccessMode);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_ImposeVisibility(NODE_HANDLE hNode, J_NODE_VISIBILITY Visibility);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetAlias(NODE_HANDLE hNode, NODE_HANDLE* phAliasNode);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_RegisterCallback(NODE_HANDLE hNode, J_NODE_CALLBACK_FUNCTION CallBackFunction);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetType(NODE_HANDLE hNode, J_NODE_TYPE* pNodeType);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetIsSelector(NODE_HANDLE hNode, uint32_t* pValue);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetNumOfSelectedFeatures(NODE_HANDLE hNode, uint32_t *pNum);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note                      Use <c>J_NodeGetIsSelector()</c> to determine if the node is a Selector node or not.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetSelectedFeatureByIndex(NODE_HANDLE hNode, uint32_t index, NODE_HANDLE * hSelectedNode);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note                      Use <c>J_NodeGetIsSelector()</c> to determine if the node is a Selector node or not.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetNumOfSelectingFeatures(NODE_HANDLE hNode, uint32_t *pNum);
...
/// \retval                     Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note                       Use <c>J_NodeGetIsSelector()</c> to determine if the node is a Selector node or not.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetSelectingFeatureByIndex(NODE_HANDLE hNode, uint32_t index, NODE_HANDLE * hSelectingNode);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_GetGenICamErrorInfo(tGenICamErrorInfo* gc);

//******************************************************************************************************************
/// \brief                     Get the Representation specified for the node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] pValue        Pointer to the variable in which the value is stored.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the Representation can be read
/// The node type has to be \c IInteger or \c IFloat 
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetRepresentation(NODE_HANDLE hNode, J_NODE_REPRESENTATION *pValue);

//******************************************************************************************************************
/// \brief                     Get the Unit specified for the node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] pBuffer       Pointer to the buffer in which the data is stored.
/// \param [in,out] pSize      Specify the size of the buffer, and then the function sets an actual size of stored data.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the Unit can be read
/// The node type has to be \c IInteger or \c IFloat 
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetUnit(NODE_HANDLE hNode, int8_t* pBuffer, uint32_t* pSize);

//******************************************************************************************************************
/// \brief                     Get the Documentation URL specified for the node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] pBuffer       Pointer to the buffer in which the data is stored.
/// \param [in,out] pSize      Specify the size of the buffer, and then the function sets an actual size of stored data.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the Documentation URL can be read
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetDocuURL(NODE_HANDLE hNode, int8_t* pBuffer, uint32_t* pSize);

//******************************************************************************************************************
/// \brief                     Get if the GenICam node is Deprecated.
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] pVal          Pointer to the variable in which the data is stored.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before it can be read
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetIsDeprecated(NODE_HANDLE hNode, uint32_t* pVal);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetMinInt64(NODE_HANDLE hNode, int64_t* pValue);
...
/// The node type has to be \c IInteger
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetMaxInt64(NODE_HANDLE hNode, int64_t* pValue);
...
/// The node type has to be \c IInteger
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetInc(NODE_HANDLE hNode,int64_t *pValue);
...
/// The node type has to be \c IInteger, \c IEnumeration or \c IBoolean
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_SetValueInt64(NODE_HANDLE hNode, bool Verify, int64_t Value);
...
/// The node type has to be \c IInteger, \c IEnumeration or \c IBoolean
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetValueInt64(NODE_HANDLE hNode, bool Verify, int64_t* pValue);

//******************************************************************************************************************
/// \brief                     Get the IFloat Alias node from the IInteger node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] phNode        Pointer to a variable in which the handle of the node object is stored.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the value can be read
/// The node type has to be \c IInteger. If no IFloat Alias node is defined the returned handle will be 0.
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetFloatAlias(NODE_HANDLE hNode, NODE_HANDLE* phNode);
...
/// The node type has to be \c IFloat
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetMinDouble(NODE_HANDLE hNode, double* pValue);
...
/// The node type has to be \c IFloat
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetMaxDouble(NODE_HANDLE hNode, double* pValue);
...
/// The node type has to be \c IFloat
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_SetValueDouble(NODE_HANDLE hNode, bool Verify, double Value);
...
/// The node type has to be \c IFloat
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetValueDouble(NODE_HANDLE hNode, bool Verify, double* pValue);

//******************************************************************************************************************
/// \brief                     Get the Display Notation specified for the IFloat node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] pValue        Pointer to the variable in which the value is stored.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the Display Notation can be read
/// The node type has to be \c IFloat 
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetDisplayNotation(NODE_HANDLE hNode, J_NODE_DISPLAY_NOTATION *pValue);

//******************************************************************************************************************
/// \brief                     Get the Display Precision specified for the IFloat node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] pValue        Pointer to the variable in which the value is stored.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the Display Precision can be read
/// The node type has to be \c IFloat 
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetDisplayPrecision(NODE_HANDLE hNode, int64_t *pValue);

//******************************************************************************************************************
/// \brief                     Inquire if the IFloat has got an Inc defined
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] pValue        Pointer to the variable in which the value is stored. Non zero if the node has got an Inc value.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before this can be read
/// The node type has to be \c IFloat 
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetFloatHasInc(NODE_HANDLE hNode, uint32_t*pValue);

//******************************************************************************************************************
/// \brief                     Get the constant increment value as double type from the IFloat node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [in] pValue         Pointer to the variable in which the value is stored.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the value can be read
/// The node type has to be \c IFloat and the \c J_Node_FloatHasInc() need to return non-zero parameter value.
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetFloatInc(NODE_HANDLE hNode, double* pValue);

//******************************************************************************************************************
/// \brief                     Get the IInteger Alias node from the IFloat node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] phNode        Pointer to a variable in which the handle of the node object is stored.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the value can be read
/// The node type has to be \c IFloat. If no IInteger Alias node is defined the returned handle will be 0.
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetIntAlias(NODE_HANDLE hNode, NODE_HANDLE* phNode);

//******************************************************************************************************************
/// \brief                     Get the IEnumeration Alias node from the IFloat node
/// \param [in] hNode          Handle to a valid node object, obtained by \c J_Camera_GetNode****, \c J_Camera_GetFeature**** or so on.
/// \param [out] phNode        Pointer to a variable in which the handle of the node object is stored.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \note
/// The camera needs to be opened using \c J_Camera_Open() before the value can be read
/// The node type has to be \c IFloat. If no IEnumeration Alias node is defined the returned handle will be 0.
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetEnumAlias(NODE_HANDLE hNode, NODE_HANDLE* phNode);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetNumOfEnumEntries(NODE_HANDLE hEnumNode, uint32_t *pNum);
...
/// The node type has to be \c J_NODE_TYPE.IEnumeration
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES, \c J_NODE_TYPE
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetEnumEntryByIndex(NODE_HANDLE hEnumNode, uint32_t index, NODE_HANDLE* hEnumEntryNode);
...
/// The node type has to be \c J_NODE_TYPE.IEnumeration
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES, \c J_NODE_TYPE
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetEnumEntryValue(NODE_HANDLE hEnumNode, int64_t* pValue);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES, \c J_NODE_TYPE
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_ExecuteCommand(NODE_HANDLE hNode);
...
/// The node type has to be \c J_NODE_TYPE.ICommand
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES, \c J_NODE_TYPE
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetCommandIsDone(NODE_HANDLE hNode, uint32_t* pValue);
...
/// The node type can be any type. The value will automatically be converted internally in the factory
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_SetValueString(NODE_HANDLE hNode, bool Verify, int8_t* ValueStr);
...
/// The node type can be any type. The value will automatically be converted internally in the factory
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetValueString(NODE_HANDLE hNode, bool Verify, int8_t* ValueStr, uint32_t* pSize);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES, \c J_NODE_TYPE
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_SetValueRegister(NODE_HANDLE hNode, const uint8_t *pBuffer, int64_t Length);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES, \c J_NODE_TYPE
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetValueRegister(NODE_HANDLE hNode, uint8_t *pBuffer, int64_t Length);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES, \c J_NODE_TYPE
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetRegisterLength(NODE_HANDLE hNode, int64_t *pValue);
...
/// \sa \c J_Camera_Open(), \c J_STATUS_CODES, \c J_NODE_TYPE
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Node_GetRegisterAddress(NODE_HANDLE hNode, int64_t *pValue);
...
{
   BAYER_STANDARD = 0,           //!< Standard 3x3 Bayer algorithm where the nearest neighbours are averaged to create color information
   BAYER_STANDARD_MULTI = 1,     //!< Standard 3x3 Bayer algorithm with multi core processing
   BAYER_EXTEND = 2,             //!< Extented 3x3 Bayer algorithm where the nearest neighbour pixels used for averaging are selected based on the gradient. This will help remove zipper-effects on horizontal and vertical edges
   BAYER_EXTEND_MULTI = 3,       //!< Extented 3x3 Bayer algorithm with multi core processing
   BAYER_SIMPLE = 4,             //!< Simple Bayer algorithm that improves processing speed over the Standard 3x3 Bayer algorithm but sacrifies the image quality. Instead of averaging neighbour pixels this algorithm will simply copy values instead.
   BAYER_SIMPLE_MULTI = 5,       //!< Simple Bayer algorithm with multi core processing
   BAYER_FAST = 6,               //!< Very fast Bayer algorithm that sacrifies the image quality as well as halfing both the horizontal and vertical resolution. This can be used in application that only display the images and where the speed is most important.
   BAYER_FAST_MULTI = 7          //!< Very fast Bayer algorithm with multi core processing
} J_BAYER_ALGORITHM;
...
/// \brief Option type used in \c J_Image_SetViewWindowOption() and \c J_Image_GetViewWindowOption()
typedef enum _J_IVW_OPTION
{
	J_IVW_MOUSE_ZOOM = 0,       //!< Enable/Disable the mouse zoom feature using \c J_Image_SetViewWindowOption(). The \c pOption parameter is a pointer to a bool value
	J_IVW_MOUSE_CURSOR = 1,     //!< Enable/Disable the mouse cursor using \c J_Image_SetViewWindowOption(). The \c pOption parameter is a pointer to a bool value
	J_IVW_CALLBACK_MASK = 2,    //!< Select which of the callback events to enable using \c J_Image_SetViewWindowOption(). The \c pOption parameter is a pointer to an integer value and the value is a bitmask created by OR'ing the \c _J_IVW_CALLBACK_TYPE enumeration values
	J_IVW_DOUBLE_BUFFERING = 3, //!< Enable/Disable Double-Buffering to enable using \c J_Image_SetViewWindowOption(). By enabling The \c pOption parameter is a pointer to a bool value
} J_IVW_OPTION;

/// \brief Image View Window Callback type. This type determines how to interprete the data passed in by the callback value parameter. It is also used when the callback mask is set as a View Window option.
typedef enum _J_IVW_CALLBACK_TYPE
{
	J_IVW_CB_WINDOW_MOVE        = 0x0001,  //!< The View Window has been moved. The pValue pointer in the callback function points to a POINT structure
	J_IVW_CB_WINDOW_SIZE        = 0x0002,  //!< The View Window has been resized. The pValue pointer in the callback function points to a SIZE structure
	J_IVW_CB_MOUSE_MOVE         = 0x0004,  //!< The mouse has been moved inside the View Window. The pValue pointer in the callback function points to a POINT structure. The location is in image pixel coordinates
	J_IVW_CB_MOUSE_LBUTTON_DOWN = 0x0008,  //!< The left mouse button has been clicked inside the View Window. The pValue pointer in the callback function points to a POINT structure. The location is in image pixel coordinates
	J_IVW_CB_MOUSE_LBUTTON_UP   = 0x0010,  //!< The left mouse button has been released inside the View Window. The pValue pointer in the callback function points to a POINT structure. The location is in image pixel coordinates
	J_IVW_CB_MOUSE_RBUTTON_DOWN = 0x0020,  //!< The right mouse button has been clicked inside the View Window. The pValue pointer in the callback function points to a POINT structure. The location is in image pixel coordinates
	J_IVW_CB_MOUSE_RBUTTON_UP   = 0x0040,  //!< The right mouse button has been released inside the View Window. The pValue pointer in the callback function points to a POINT structure. The location is in image pixel coordinates
    J_IVW_CB_ZOOM_POS           = 0x0080,  //!< The Zoom or view location has changed the View Window. The pValue pointer returns the new view position and zoom values in a J_IVW_ZOOM_POS structure
    J_IVW_CB_AREA_SELECT        = 0x0100,  //!< The user has dragged a selection rectangle using the mouse. The pValue pointer returns the selected rectangle in a RECT structure
    J_IVW_CB_USER_PAINT_IMAGE   = 0x0200,  //!< The VisionLib is done painting. The user can now paint something on top of the image (such as text, graphics, ...)
    J_IVW_CB_USER_PAINT_WINDOW  = 0x0400,  //!< The VisionLib is done painting. The user can now paint something on top of the View Window (such as text, graphics, ...)
} J_IVW_CALLBACK_TYPE;

typedef CJDummyClass * J_IVW_CALLBACK_OBJECT;
#ifdef __BORLANDC__
   // The Borland C++ Builder does not work with the Class callback functions because
   // the JAI SDK is created using Microsoft Visual C/C++ compiler!
   // This means that the callback function has to be a static function!
   typedef void (__stdcall*J_IVW_CALLBACK_FUNCTION)(VIEW_HANDLE hWin, J_IVW_CALLBACK_TYPE Type, void *pValue);
#else
   typedef void (CJDummyClass::*J_IVW_CALLBACK_FUNCTION)(VIEW_HANDLE hWin, J_IVW_CALLBACK_TYPE Type, void *pValue);
#endif

#ifdef __linux__
   typedef void ( *J_STATIC_IVW_CALLBACK_FUNCTION)(VIEW_HANDLE hWin, J_IVW_CALLBACK_TYPE Type, void *pValue);
#else
   typedef void (__stdcall *J_STATIC_IVW_CALLBACK_FUNCTION)(VIEW_HANDLE hWin, J_IVW_CALLBACK_TYPE Type, void *pValue);
#endif

/// \brief View Window Callback datatype for the image Zoom changes. This structure is used by the View Window callback notification when the pan or zoom changes
typedef struct tagJViewWindowZoomPosition
{
    POINT  Offset;                         //!< Offset inside the View Window
    double ZoomRatioX;                     //!< Zoom ratio in X-direction.
    double ZoomRatioY;                     //!< Zoom ratio in Y-direction.
} J_IVW_ZOOM_POS, *PJ_IVW_ZOOM_POS;

/// \brief View Window Callback datatype for the User-Draw overlay callbacks. This structure is used by the View Window callback notification when the text and graphics overlay can be added.
typedef struct tagJViewWindowUserPaint
{
    HDC     hDC;                           //!< Handle to the Device Context to be used for the overlay drawing
    int32_t ImageWidth;                    //!< Width of the User-Draw overlay area that can be drawn onto
    int32_t ImageHeight;                   //!< Height of the User-Draw overlay area that can be drawn onto
} J_IVW_USER_PAINT, *PJ_IVW_USER_PAINT;


#ifdef UNICODE
#define J_Image_OpenViewWindow  J_Image_OpenViewWindowW
#else
#define J_Image_OpenViewWindow  J_Image_OpenViewWindowA
#endif // !UNICODE

//******************************************************************************************************************
/// \brief   Open View Window (Create)
///
/// A window of size MaxSize is created for the client area. When the images are displayed using \c J_Image_ShowImage() 
/// the view window will automatically be resized. 
///
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Image_OpenViewWindow definition. This will automatically select the appropriate version of the function.
/// \param [in] pWindowName   Name to be displayed in the view window caption. The Window Name is a 8-bit (ANSI) character string.
...
///
/// // Open view window
/// retval = J_Image_OpenViewWindow(_T("Live view!"), &TopLeft, &ViewSize, &hView);
/// if (retval == J_ST_SUCCESS)
/// {
///   // Open stream
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_OpenViewWindowA(LPCSTR pWindowName, POINT *pPoint, SIZE *pMaxSize, VIEW_HANDLE *pWin);

//******************************************************************************************************************
/// \brief   Open View Window (Create)
///
/// A window of size MaxSize is created for the client area. When the images are displayed using \c J_Image_ShowImage() the 
/// view window will automatically be resized. 
///
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Image_OpenViewWindow definition. This will automatically select the appropriate version of the function.
/// \param [in] pWindowName   Name to be displayed in the view window caption. The Window Name is an Unicode character string.
/// \param [in] *pPoint       Point of the upper left corner of the view window (absolute coordinate).
/// \param [in] *pMaxSize     Maximum size of the view window
/// \param [out] *pWin        The handle of the created window (window object) is returned.
/// \retval                   Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
...
/// \endcode
/// \sa \c J_Image_OpenViewWindow(), \c J_Camera_GetNodeByName(), \c J_Node_ExecuteCommand(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_OpenViewWindowW(LPCWSTR pWindowName, POINT *pPoint, SIZE *pMaxSize, VIEW_HANDLE *pWin);

#ifdef UNICODE
#define J_Image_OpenViewWindowEx  J_Image_OpenViewWindowExW
#else
#define J_Image_OpenViewWindowEx  J_Image_OpenViewWindowExA
#endif // !UNICODE

//******************************************************************************************************************
/// \brief   Open View Window Ex (Create)
/// 
/// A window of size MaxSize is created for the client area. When the images are displayed using \c J_Image_ShowImage() the view window will automatically be resized.
...
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Image_OpenViewWindowEx definition. This will automatically select the appropriate version of the function.
/// \param  [in] iWindowType   Window type
/// \param  [in] pWindowName   Window name. The Window Name is a 8-bit (ANSI) character string.
...
/// \param  [out] *pWin        The handle of the created window (window object) is returned.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_OpenViewWindowExA(J_IVW_WINDOW_TYPE iWindowType, LPCSTR pWindowName, RECT *pFrameRect, SIZE *pMaxSize, HWND hParent, VIEW_HANDLE *pWin);
...
/// \c J_Image_OpenViewWindowEx definition. This will automatically select the appropriate version of the function.
/// \param  [in] iWindowType   Window type
/// \param  [in] pWindowName   Window name. The Window Name is an Unicode character string.
/// \param  [in] *pFrameRect   Externals size of frame window(absolute coordinate).
/// \param  [in] *pMaxSize     Maximum size of image data.
/// \param  [in] hParent       Handle to parent window.
/// \param  [out] *pWin        The handle of the created window (window object) is returned.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_OpenViewWindowExW(J_IVW_WINDOW_TYPE iWindowType, LPCWSTR pWindowName, RECT *pFrameRect, SIZE *pMaxSize, HWND hParent, VIEW_HANDLE *pWin);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_ShowImage(VIEW_HANDLE hWin, J_tIMAGE_INFO * pAqImageInfo, uint32_t iRGain=0x01000, uint32_t iGGain=0x01000, uint32_t iBGain=0x01000);
...
/// \param [in] iBGain           Gain for the blue color channel for Bayer conversion (0x01000(4096) = 1.00)
/// \retval                      Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_ShowImageEx(VIEW_HANDLE hWin, J_tIMAGE_INFO * pAqImageInfo, J_BAYER_ALGORITHM iBayerAlgorithm = BAYER_STANDARD, uint32_t iRGain=0x01000, uint32_t iGGain=0x01000, uint32_t iBGain=0x01000);
...
/// \retval                Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SetImageOffset(VIEW_HANDLE hWin, POINT *pOffset, int32_t ZoomRatio);

//******************************************************************************************************************
/// \brief   Set image offset and zoom ratio (individually in both X and Y direction).
///
/// Sets the offset and the zoom ratio of the view window image. 
///
/// \param [in] hWin       Handle to a view window previously opened using \c J_Image_OpenViewWindow() or \c J_Image_OpenViewWindowEx()
/// \param [in] *pOffset   POINT structure pointer with the horizontal, vertical offset (number of pixels).
/// \param [in] ZoomRatioX Zoom ratio in X-direction.
/// \param [in] ZoomRatioY Zoom ratio in Y-direction.
///                        100.0:The original image is reduced to the window size.
///                        1.0  :1/100 of the original images is expanded to the window size. 
/// \retval                Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SetImageOffsetEx(VIEW_HANDLE hWin, POINT *pOffset, double ZoomRatioX, double ZoomRatioY);

//******************************************************************************************************************
/// \brief   Get the current image offset and zoom ratio (individually in both X and Y direction).
///
/// Gets the current offset and the zoom ratio of the view window image. 
///
/// \param [in] hWin          Handle to a view window previously opened using \c J_Image_OpenViewWindow() or \c J_Image_OpenViewWindowEx()
/// \param [in] *pOffset      POINT structure pointer with the horizontal, vertical offset (number of pixels).
/// \param [out] *pZoomRatioX Zoom ratio in X-direction.
/// \param [out] *pZoomRatioY Zoom ratio in Y-direction.
///                           100.0:The original image is reduced to the window size.
///                           1.0  :1/100 of the original images is expanded to the window size. 
/// \retval                   Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetImageOffsetEx(VIEW_HANDLE hWin, POINT *pOffset, double *pZoomRatioX, double *pZoomRatioY);
...
/// \retval               Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_MoveViewWindow(VIEW_HANDLE hWin, POINT *pPoint);
...
/// \retval               Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_ResizeChildWindow(VIEW_HANDLE hWin, RECT *pRect);
...
/// \retval               Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetViewWindowRect(VIEW_HANDLE hWin, RECT *pRect);
...
/// \param [out] *pRect   Present window coordinates.
/// \retval               Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetFrameWindowRect(VIEW_HANDLE hWin, RECT *pRect);

#ifdef UNICODE
#define J_Image_SetViewWindowTitle  J_Image_SetViewWindowTitleW
#else
#define J_Image_SetViewWindowTitle  J_Image_SetViewWindowTitleA
#endif // !UNICODE
...
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Image_SetViewWindowTitle definition. This will automatically select the appropriate version of the function.
/// \param [in] hWin           Handle to a view window previously opened using \c J_Image_OpenViewWindow() or \c J_Image_OpenViewWindowEx()
/// \param [in] *pWindowTitle  Character string that wants to be displayed in title bar. The Window Name is a 8-bit (ANSI) character string.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SetViewWindowTitleA(VIEW_HANDLE hWin, LPCSTR pWindowTitle);
...
/// \c J_Image_SetViewWindowTitle definition. This will automatically select the appropriate version of the function.
/// \param [in] hWin           Handle to a view window previously opened using \c J_Image_OpenViewWindow() or \c J_Image_OpenViewWindowEx()
/// \param [in] *pWindowTitle  Character string that wants to be displayed in title bar. The Window Name is an Unicode character string.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SetViewWindowTitleW(VIEW_HANDLE hWin, LPCWSTR pWindowTitle);

//******************************************************************************************************************
/// \brief   Set the value for a View Window option.
/// 
/// Modify of the options for the view window 
///
/// \param [in] hWin           Handle to a view window previously opened using \c J_Image_OpenViewWindow() or \c J_Image_OpenViewWindowEx()
/// \param [in] Option         Option to be set. This has to be one of the J_IVW_OPTION enumeration values.
/// \param [in] *pOptionValue  Pointer to a variable containing the Value to be set. The type of the value variable depend on the Option. See the
///                            documentation for the \c J_IVW_OPTION for more details.
/// \retval                    Status code defined in the \c J_STATUS_CODES. If the function succeeds, returns \c J_ST_SUCCESS.
/// \par
/// \code
...
/// 
///   // Open view window
///   if (J_Image_OpenViewWindow(_T("Live view!"),
///                              &TopLeft,
///                              &ViewSize,
///                              &g_hView) != J_ST_SUCCESS)
///     return FALSE;
/// 
///   // Enable the Mouse Zoom option
///   bool EnableZoomOption = true;
///   if (J_Image_SetViewWindowOption(g_hView, J_IVW_MOUSE_ZOOM, &EnableZoomOption) != J_ST_SUCCESS)
///     return FALSE;
/// \endcode
/// \sa \c J_Image_OpenViewWindow(), \c J_Camera_GetNodeByName(), \c J_Node_ExecuteCommand(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SetViewWindowOption(VIEW_HANDLE hWin, J_IVW_OPTION Option, void* pOptionValue);

//******************************************************************************************************************
/// \brief   Get the current View Window option value
/// 
/// Read the value of the options for the view window 
///
/// \param [in] hWin           Handle to a view window previously opened using \c J_Image_OpenViewWindow() or \c J_Image_OpenViewWindowEx()
/// \param [in] Option         Option to be set. This has to be one of the J_IVW_OPTION enumeration values.
/// \param [out] *pOptionValue Pointer to a variable to be asigned with the Value of the option. The type of the value variable depend on the Option type.
///                            See the documentation for the \c J_IVW_OPTION enumeration for more details.
/// \retval                    Status code defined in the \c J_STATUS_CODES. If the function succeeds, returns \c J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetViewWindowOption(VIEW_HANDLE hWin, J_IVW_OPTION Option, void* pOptionValue);

//******************************************************************************************************************
/// \brief   Set the callback routine to be used for the View Window notification handling.
/// 
/// Set up a callback routine that will notify the user application about events such as mouse move events. See the documentation for the \c _J_IVW_CALLBACK_TYPE 
///
/// \param [in] hWin        Handle to a view window previously opened using \c J_Image_OpenViewWindow() or \c J_Image_OpenViewWindowEx()
...
/// static void __stdcall ViewWindowCBFunc(VW_HANDLE hWin, J_IVW_CALLBACK_TYPE Type, void *pValue)
/// {
///   switch (Type)
///   {
///     case IVW_CB_WINDOW_MOVE:
///        // Do something with the Window Move notification
///        // The pValue is then a pointer to a POINT structure
///        // x = *((POINT*)pValue).x;
///        // y = *((POINT*)pValue).y;
///     break;
///     case IVW_CB_WINDOW_SIZE:
///        // Do something with the Window Size notification
///        // The pValue is then a pointer to a SIZE structure
///        // width = *((SIZE*)pValue).cx;
///        // height = *((SIZE*)pValue).cy;
///     break;
///     case IVW_CB_MOUSE_MOVE:
///        // Do something with the Mouse Move notification
///        // The pValue is then a pointer to a POINT structure and it is the pixel location inside the image (not the screen position!)
///        // x = *((POINT*)pValue).x;
///        // y = *((POINT*)pValue).y;
///     break;
///   }
...
///     return FALSE;
/// 
///   // Assign the View Window callback
///   void *vfptr1 = reinterpret_cast<void*>(ViewWindowCBFunc);
///   J_IVW_CALLBACK_FUNCTION *cbfptr1 =  reinterpret_cast<J_IVW_CALLBACK_FUNCTION*>(&vfptr1);
///
///   if (J_Image_SetViewWindowCallback(g_hView, NULL, *cbfptr1) != J_ST_SUCCESS)
///     return FALSE;
/// 
///   // Enable the Mouse Zoom option
///   bool EnableZoomOption = true;
...
///     return FALSE;
///
///   // Enable the Mouse Move notification option
///   uint32_t CallbackMask = IVW_CB_MOUSE_MOVE;
///   if (J_Image_SetViewWindowOption(g_hView, J_IVW_CALLBACK_MASK, &CallbackMask) != J_ST_SUCCESS)
///     return FALSE;
///
///   // Open stream
///   void *vfptr2 = reinterpret_cast<void*>(StreamCBFunc);
///   J_IMG_CALLBACK_FUNCTION *cbfptr2 =  reinterpret_cast<J_IMG_CALLBACK_FUNCTION*>(&vfptr2);
///
...
/// \endcode
/// \sa \c J_Image_OpenViewWindow(), \c J_Camera_GetNodeByName(), \c J_Node_ExecuteCommand(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SetViewWindowCallback(VIEW_HANDLE hWin, J_IVW_CALLBACK_OBJECT CBObject, J_IVW_CALLBACK_FUNCTION CBFunction);
...
/// \endcode
/// \sa \c J_Image_OpenViewWindow(), \c J_Camera_GetNodeByName(), \c J_Node_ExecuteCommand(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_OpenStream(CAM_HANDLE hCam, uint32_t iChannel, J_IMG_CALLBACK_OBJECT CBObject, J_IMG_CALLBACK_FUNCTION CBFunction, THRD_HANDLE * phThread, uint32_t iBufferSize=(J_XGA_WIDTH * J_XGA_HEIGHT * J_MAX_BPP), DWORD iMcIP=0);
...
/// \endcode
/// \sa \c J_Image_OpenViewWindow(), \c J_Camera_GetNodeByName(), \c J_Node_ExecuteCommand(), \c J_STATUS_CODES
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_OpenStreamLight(CAM_HANDLE hCam, uint32_t iChannel,  THRD_HANDLE* phThread);
...
/// \retval               Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_CloseStream(THRD_HANDLE hThread);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetStreamInfo(THRD_HANDLE hThread, J_STREAM_INFO_CMD iCmd, void *pBuffer, uint32_t *pSize);

//******************************************************************************************************************
/// \brief                      Get stream handle from internal image stream
/// \param [in] hThread         Handle to a valid thread object, obtained by \c J_Image_OpenStream() or \c J_Image_OpenStreamLight() function.
/// \param [in,out] phStream    Pointer to STREAM_HANDLE variable where the handle is returned.
/// \retval                     Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Image_OpenStream(), \c J_Image_OpenStreamLight(), \c J_STREAM_INFO_CMD
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetStreamHandle(THRD_HANDLE hThread, STREAM_HANDLE *phStream);
...
/// \brief Image conversion options used in \c J_Image_ConvertImage() and \c J_Image_MallocEx()
typedef enum _J_PIXEL_FORMAT_TYPE
{
   PF_NONE = 0,              //!< Only used in J_Image_MallocEx() to allocate a buffer of similar size as original
   ...
   PF_SWAP_RB,               //!< Selects the output bit-length automatically. (24bit->24bit, 48bit->48bit)
   PF_Y,                     //!< Selects the outputbit-length automatically. (24bit->8bit, 48bit->16bit)
   PF_3Y_H,                  //!< Convert 24-bit RGB/24-bit BGR/30-bit RGB Packed V1/30-bit RGB Packed V2) into 3xY images stitched together horizontally (3-head camera special feature!)
   PF_3Y_V,                  //!< Convert 24-bit RGB/24-bit BGR/30-bit RGB Packed V1/30-bit RGB Packed V2) into 3xY images stitched together vertically (3-head camera special feature!)
} J_PIXEL_FORMAT;
...
/// \brief Structure used for accessing different pixel values when using \c J_Image_GetPixel() and  \c J_Image_GetPixelEx()
...
/// \retval                     Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_FromRawToDIB(J_tIMAGE_INFO * pAqImageInfo, J_tIMAGE_INFO * pBufferInfo, uint32_t iRGain=0x01000, uint32_t iGGain=0x01000, uint32_t iBGain=0x01000);
...
/// \param [in] iBGain          B Gain for Bayer (0x01000(4096) = 1.00)
/// \retval                     Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_FromRawToDIBEx(J_tIMAGE_INFO * pAqImageInfo, J_tIMAGE_INFO * pBufferInfo, J_BAYER_ALGORITHM iBayerAlgorithm = BAYER_STANDARD, uint32_t iRGain=0x01000, uint32_t iGGain=0x01000, uint32_t iBGain=0x01000);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_FromRawToImage(J_tIMAGE_INFO * pAqImageInfo, J_tIMAGE_INFO * pBufferInfo, uint32_t iRGain=0x01000, uint32_t iGGain=0x01000, uint32_t iBGain=0x01000);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_FromRawToImageEx(J_tIMAGE_INFO * pAqImageInfo, J_tIMAGE_INFO * pBufferInfo, J_BAYER_ALGORITHM iBayerAlgorithm = BAYER_STANDARD, uint32_t iRGain=0x01000, uint32_t iGGain=0x01000, uint32_t iBGain=0x01000);

//******************************************************************************************************************
/// \brief Convert an image into some other format such as Color to Monochrome conversion.
/// 
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_ConvertImage(J_tIMAGE_INFO * pInputBufferInfo, J_tIMAGE_INFO * pOutputBufferInfo, J_PIXEL_FORMAT iOutputFormat);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_Malloc(J_tIMAGE_INFO * pAqImageInfo, J_tIMAGE_INFO * pBufferInfo);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_MallocDIB(J_tIMAGE_INFO * pAqImageInfo, J_tIMAGE_INFO * pBufferInfo);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_MallocEx(J_tIMAGE_INFO * pInputBufferInfo, J_tIMAGE_INFO * pOutputBufferInfo, J_PIXEL_FORMAT iOutputFormat);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_Free(J_tIMAGE_INFO * pBufferInfo);

/// @}
/// \defgroup WRAP_IMAGE_save   Image save and load functions
/// \ingroup WRAP_IMAGE
/// @{
/// \brief These functions are used for saving the acquired images to disk as well as loading previously saved images. 
/// \par
...
/// 4) JAI: This is an uncompressed RAW file format that support all the GigE Vision file image formats supported by the JAI SDK \n
/// \par
/// The \c J_Image_SaveFileRaw() function will save the contents of the acquired image \c without any image file header information - just the RAW data
/// received from the camera.
/// \par
/// The \c J_Image_LoadFileRaw() function will be able to load a RAW image file that has been saved with either \c J_Image_SaveFileEx() in \c JAI
/// format or an image saved using \c J_Image_SaveFileRaw().
...
typedef enum
{
   ...
   J_FF_JAI,         //!< Proprietary RAW file format that includes the RAW pixel data as well as additional image informations
   J_FF_RAW,         //!< RAW file format that only contains the pixel data. This is identical to using the \c J_Image_SaveFileRaw() function
} J_SIF_FILE_FORMAT;

#ifdef UNICODE
#define J_Image_SaveFile  J_Image_SaveFileW
#else
#define J_Image_SaveFile  J_Image_SaveFileA
#endif // !UNICODE
...
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Image_SaveFile definition. This will automatically select the appropriate version of the function.
...
/// \param [in] pPath        Full file name and path of the tiff file to be stored to disk. The file Name is a 8-bit (ANSI) character string.
/// \retval                  Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \par
/// \code
/// // C Sample that converts a raw image and saved the result to a file
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SaveFileA(J_tIMAGE_INFO * pBufferInfo, LPCSTR pPath);
...
/// \c J_Image_SaveFile definition. This will automatically select the appropriate version of the function.
/// \param [in] pBufferInfo  Pointer of J_tIMAGE_INFO structure in which the information about the allocated buffer will be stored.
/// \param [in] pPath        Full file name and path of the tiff file to be stored to disk. The file Name is an Unicode character string.
/// \retval                  Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \par
/// \code
/// // C Sample that converts a raw image and saved the result to a file
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SaveFileW(J_tIMAGE_INFO * pBufferInfo, LPCWSTR pPath);

#ifdef UNICODE
#define J_Image_SaveFileEx  J_Image_SaveFileExW
#else
#define J_Image_SaveFileEx  J_Image_SaveFileExA
#endif // !UNICODE

//******************************************************************************************************************
/// \brief   Save an image to disk as TIFF, JPEG, BMP or JAI file
/// 
/// Saves an image to disk as a TIFF, JPEG, BMP or JAI file. A raw image needs to be converted using \c J_Image_FromRawToImage() before it can be saved
/// unless the JAI file format is being used.
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Image_SaveFileEx definition. This will automatically select the appropriate version of the function.
/// \param [in] pBufferInfo       Pointer of J_tIMAGE_INFO structure in which the information about the allocated buffer will be stored.
/// \param [in] pPath             Full file name and path of the tiff file to be stored to disk. The file Name is a 8-bit (ANSI) character string.
...
/// }
///
/// // C Sample that saves the image to file in raw image format that can later be read back using \c J_Image_LoadFileRaw()
/// BOOL SaveJAIImageToFile(J_tIMAGE_INFO* ptRawImageInfo, LPCTSTR sFileName)
/// {
///   // Save the image to disk in TIFF format
///   if (J_Image_SaveFileEx(&tCnvImageInfo, sFileName, J_FF_JAI) != J_ST_SUCCESS)
///     return FALSE;
///   return TRUE;
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SaveFileExA(J_tIMAGE_INFO * pBufferInfo, LPCSTR pPath, J_SIF_FILE_FORMAT iFileFormat=J_FF_TIFF,  uint8_t iEncoderParameter=75);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SaveFileExW(J_tIMAGE_INFO * pBufferInfo, LPCWSTR pPath, J_SIF_FILE_FORMAT iFileFormat=J_FF_TIFF,  uint8_t iEncoderParameter=75);

#ifdef UNICODE
#define J_Image_SaveFileRaw  J_Image_SaveFileRawW
#else
#define J_Image_SaveFileRaw  J_Image_SaveFileRawA
#endif // !UNICODE
...
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Image_SaveFileRaw definition. This will automatically select the appropriate version of the function.
/// \param [in] pBufferInfo  Pointer of J_tIMAGE_INFO structure in which the raw image data is read.
/// \param [in] pPath        Full file name and path of the raw binary file to be stored to disk. The file Name is a 8-bit (ANSI) character string.
/// \retval                  Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \par
/// \code
...
///   if (J_Image_SaveFileRaw(ptRawImageInfo, sFileName) != J_ST_SUCCESS)
///     return FALSE;
/// 
///   return TRUE;
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SaveFileRawA(J_tIMAGE_INFO * pBufferInfo, LPCSTR pPath);
...
/// \c J_Image_SaveFileRaw definition. This will automatically select the appropriate version of the function.
/// \param [in] pBufferInfo  Pointer of J_tIMAGE_INFO structure in which the raw image data is read.
/// \param [in] pPath        Full file name and path of the raw binary file to be stored to disk. The file Name is an Unicode character string.
/// \retval                  Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \par
/// \code
/// // C Sample that saves a raw image to a file
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SaveFileRawW(J_tIMAGE_INFO * pBufferInfo, LPCWSTR pPath);

#ifdef UNICODE
#define J_Image_LoadFileRaw  J_Image_LoadFileRawW
#else
#define J_Image_LoadFileRaw  J_Image_LoadFileRawA
#endif // !UNICODE

//******************************************************************************************************************
/// \brief   Load a previously saved RAW image file from disk.
/// 
/// If the file was saved using \c J_Image_SaveFileEx() in \c JAI file format then the \c pBufferInfo structure parameter will
/// automatically be initialized with the proper values from the original image that was saved.
/// If the file was saved using \c J_Image_SaveFileRaw() then the \c pBufferInfo structure parameter will need to be
/// initialized with the proper values by the application!.
///
/// \note
/// This function will automaticall allocated the correct amount of memory needed to hold the image data. Please remember to
/// call \c J_Image_Free(pBufferInfo) when the image is not needed any more!
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Image_LoadFileRaw definition. This will automatically select the appropriate version of the function.
/// \param [in] pBufferInfo  Pointer of J_tIMAGE_INFO structure in which the raw image data is to e stored.
/// \param [in] pPath        Full file name and path of the raw binary file to be loaded from disk. The file Name is a 8-bit (ANSI) character string.
/// \retval                  Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \par
/// \code
/// // C Sample that loads a raw image from a file
/// BOOL LoadImageFromRawBinaryFile(J_tIMAGE_INFO* ptRawImageInfo, LPCTSTR sFileName)
/// {
///   // Load the image from disk in raw binary format
///   if (J_Image_LoadFileRaw(&tCnvImageInfo, sFileName) != J_ST_SUCCESS)
///     return FALSE;
/// 
///   return TRUE;
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_LoadFileRawA(J_tIMAGE_INFO * pBufferInfo, LPCSTR pPath);
...
/// automatically be initialized with the proper values from the original image that was saved.
/// If the file was saved using \c J_Image_SaveFileRaw() then the \c pBufferInfo structure parameter will need to be
/// initialized with the proper values by the applicaiton!.
///
/// \note
/// This function will automaticall allocated the correct amount of memory needed to hold the image data. Please remember to
/// call \c J_Image_Free(pBufferInfo) when the image is not needed any more!
...
/// \c J_Image_LoadFileRaw definition. This will automatically select the appropriate version of the function.
/// \param [in] pBufferInfo  Pointer of J_tIMAGE_INFO structure in which the raw image data is to e stored.
/// \param [in] pPath        Full file name and path of the raw binary file to be loaded from disk. The file Name is an Unicode character string.
/// \retval                  Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \par
/// \code
/// // C Sample that loads a raw image from a file
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_LoadFileRawW(J_tIMAGE_INFO * pBufferInfo, LPCWSTR pPath);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetPixel(J_tIMAGE_INFO * pBufferInfo, POINT *pPoint, void * pPixel);

//******************************************************************************************************************
/// \brief   Get the pixel at a specified floating-point position. The values is created using bi-linear interpolation
/// 
/// Read the current pixel value at the floating-point position \c dXPos, \c dYPos. A raw image needs to be converted using \c J_Image_FromRawToImage() before pixels can be read.
///
/// \param [in] pBufferInfo  J_tIMAGE_INFO structure in which the pixel value will be read.
/// \param [in] dXPos        Absolute position in which pixel value is read.
/// \param [in] dYPos        Absolute position in which pixel value is read.
/// \param [out] pPixel      Pixel value data read from the image.
///                          Where to get the actual values from the \c Pixel Value structure depends on the \c PixelFormat that can be read from \c pBufferInfo.
/// \retval                  Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_tIMAGE_INFO
/// \par
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetPixelEx(J_tIMAGE_INFO * pBufferInfo, double dXPos, double dYPos, void * pPixel);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_tIMAGE_INFO
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SetPixel(J_tIMAGE_INFO * pBufferInfo, POINT *pPoint, void * pPixel);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetAverage(J_tIMAGE_INFO * pBufferInfo, RECT *pMeasureRect, J_tBGR48 *pRGBAverage);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE    __cdecl J_Image_SetGain(HANDLE hThread, uint32_t iRGain, uint32_t iGGain, uint32_t iBGain);
...
/// \retval                 Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE    __cdecl J_Image_GetGain(HANDLE hThread, uint32_t * piRGain, uint32_t * piGGain, uint32_t * piBGain);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE    __cdecl J_Image_ExecuteWhiteBalance(THRD_HANDLE hThread);
...
#ifdef UNICODE
#define J_Camera_SaveSettings  J_Camera_SaveSettingsW
#else
#define J_Camera_SaveSettings  J_Camera_SaveSettingsA
#endif // !UNICODE
...
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Camera_SaveSettings definition. This will automatically select the appropriate version of the function.
/// \param [in] hCam              Handle to a valid camera object, obtained by \c J_Camera_Open() function.
/// \param [in] sSettingsFileName Zero terminated string containing the file name for the settings file. The file Name is a 8-bit (ANSI) character string.
...
/// retval = J_Camera_SaveSettings(m_hCam, _T("CameraSettings.script"));
///
...
///                                SAVE_FORCE_ALL);
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_SaveSettingsA(CAM_HANDLE hCam, LPCSTR sSettingsFileName, J_SAVE_SETTINGS_FLAG saveFlag = SAVE_AUTO);
...
/// \c J_Camera_SaveSettings definition. This will automatically select the appropriate version of the function.
/// \param [in] hCam              Handle to a valid camera object, obtained by \c J_Camera_Open() function.
/// \param [in] sSettingsFileName Zero terminated string containing the file name for the settings file. The file Name is an Unicode character string.
/// \param [in] saveFlag          Enumeration value that selects the behaviour of the Save function. See \c J_SAVE_SETTINGS_FLAG for more information.
/// \retval                 Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \code
/// // These small samples illustrates how to save setting from a connected 
...
///                                SAVE_FORCE_ALL);
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_SaveSettingsW(CAM_HANDLE hCam, LPCWSTR sSettingsFileName, J_SAVE_SETTINGS_FLAG saveFlag = SAVE_AUTO);

#ifdef UNICODE
#define J_Camera_LoadSettings  J_Camera_LoadSettingsW
#else
#define J_Camera_LoadSettings  J_Camera_LoadSettingsA
#endif // !UNICODE
...
/// \note
/// In order to use the same code in both 8-bit (ANSI) string applications and Unicode string applications please use the 
/// \c J_Camera_LoadSettings definition. This will automatically select the appropriate version of the function.
/// \param [in] hCam              Handle to a valid camera object, obtained by \c J_Camera_Open() function.
/// \param [in] sSettingsFileName Zero terminated string containing the file name for the settings file. The file Name is a 8-bit (ANSI) character string.
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_LoadSettingsA(CAM_HANDLE hCam, LPCSTR sSettingsFileName, J_LOAD_SETTINGS_FLAG loadFlag = LOAD_AUTO);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_LoadSettingsW(CAM_HANDLE hCam, LPCWSTR sSettingsFileName, J_LOAD_SETTINGS_FLAG loadFlag = LOAD_AUTO);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Camera_GetSettingsValidationErrorInfo(CAM_HANDLE hCam, int8_t* errorInfoBuffer, uint32_t *pBufferSize);
...
/// \note The input images needs to be Mono8, Mono10, Mono12, Mono14 or Mono16 pixel format for monochrome images or Bayer8, Bayer10,
/// Bayer12 or Bayer16 for CFA color images or RBG8Packed, RGB10Packed, RGB12Packed or RGB16Packed for color images. It is also required
/// that the two images has got identical pixel format
///
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_MallocHDR(J_tIMAGE_INFO * pImageInfo1, J_tIMAGE_INFO * pImageInfo2, J_tIMAGE_INFO * pBufferInfo);
...
/// \retval                         Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_AnalyzeHDR(J_tIMAGE_INFO * pImageInfo1, J_tIMAGE_INFO * pImageInfo2, uint8_t* pImage1IsBrighter, int32_t *pBlackLevelImage1, int32_t *pBlackLevelImage2, float* pDarkGain);
...
/// \retval                           Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_FuseHDR(J_tIMAGE_INFO* pBrighterImageInfo, J_tIMAGE_INFO* pDarkerImageInfo, J_tIMAGE_INFO* pOutputBufferInfo, int32_t brighterBlackLevel, int32_t darkerBlackLevel, float fDarkGain, float fDualSlopeGain, uint8_t logOutput);
...
/// \retval                      Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_MallocLUT(J_tIMAGE_INFO* pImageInfo, J_tLUT* pLutInfo);
...
/// \param [in] pLutInfo       Pointer to J_tLUT structure with information on LUT.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_FreeLUT(J_tLUT* pLutInfo);
...
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_ConvertLUT(J_tLUT* pSourceLutInfo, J_tLUT* pDestinationLutInfo);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_LUT(J_tIMAGE_INFO* pImageInfo, J_tLUT* pLutInfo);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_CreateGammaLUT(J_tLUT* pLutInfo, uint32_t ColorIndex, double gamma);
...
/// \param [in] gamma          Gamma value to be used for the creation of the LUT entries.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_ApplyGammaToLUT(J_tLUT* pLutInfo, uint32_t ColorIndex, double gamma);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_CreateLinearLUT(J_tLUT* pLutInfo, uint32_t ColorIndex, uint32_t X1, uint32_t X2, uint32_t Y1, uint32_t Y2);
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_CreateKneeLUT(J_tLUT* pLutInfo, uint32_t ColorIndex, POINT knee1, POINT knee2);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetLUTValue(J_tLUT* pLutInfo, uint32_t ColorIndex, uint32_t LUTIndex, uint32_t* pLUTValue);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SetLUTValue(J_tLUT* pLutInfo, uint32_t ColorIndex, uint32_t LUTIndex, uint32_t LUTValue);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SaveLUTA(J_tLUT* pLutInfo, LPCSTR pPath);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_SaveLUTW(J_tLUT* pLutInfo, LPCWSTR pPath);
...
///                            string.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_LoadLUTA(J_tLUT* pLutInfo, LPCSTR pPath);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_LoadLUTW(J_tLUT* pLutInfo, LPCWSTR pPath);
...
/// \retval                      Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_MallocHistogram(J_tIMAGE_INFO* pImageInfo, J_tHIST* pHistogramInfo);
...
/// \param [in] pHistogramInfo Pointer to J_tHIST structure with information on Histogram.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_FreeHistogram(J_tHIST* pHistogramInfo);
...
/// \param [in] pHistogramInfo Pointer to J_tHIST structure with information on Histogram.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_ClearHistogram(J_tHIST* pHistogramInfo);
...
/// \param [in] pHistogramInfo   Pointer to J_tHIST structure with information on Histogram.
/// \retval                      Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_CreateHistogram(J_tIMAGE_INFO* pImageInfo, RECT *pMeasureRect, J_tHIST* pHistogramInfo);
...
/// \retval                     Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_GetHistogramValue(J_tHIST* pHistogramInfo, uint32_t ColorIndex, uint32_t HistogramIndex, uint32_t* pHistogramValue);
...
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_Flip(J_tIMAGE_INFO* pImageInfo, J_IMAGE_FLIP_TYPE flipType);
...
///                            upside-down.
/// \retval                    Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_Rotate(J_tIMAGE_INFO* pImageInfo, J_IMAGE_ROTATE_TYPE rotateType);

/// @}
/// \defgroup WRAP_ACTION_COMMAND   Action Command functions
/// \ingroup WRAP_FACTORY
/// @{
/// \brief Triggering an action in multiple devices at roughly the same time can be accomplished through the optional Action
/// command introduced in GigE Vision 1.1. Using a distinct command (instead of a Write Register Command) has the 
/// advantage that it does not rely on a specific register map. Therefor, different type of devices can be targeted 
/// with the same message. Note that due to the nature of Ethernet, this is not as synchronous as a hardware trigger
/// since different network segments can have different latencies. Nevertheless in a swithced network, the resulting 
/// jitter is acceptable for a broad range of applications and this scheme provides a convenient way to synchronize 
/// devices by means of a software command.
/// The most typical scenario is when an application desires to trigger a simultaneous action on multiple devices.
/// This could for instance be triggering the device or resetting the internal timestamp counter.
/// \par
...
/// 2) The iDeviceKey in the Action Command packet and the \c ActionDeviceKey configured in the boostrap register must be equal.\n
/// 3) The iGroupKey in the Action Command packet and the \c ActionGroupKey of the corresponding device action must be equal.\n
/// 4) The logical AND-wise operation of the iGroupMask in the Action Command packet and the \c ActionGroupMask of the 
/// corresponding device action must be non-zero. That is, that they must have at least one common bit set at the same 
/// position in the 32-bit register.\n
/// \note Actions are not available in GigE Vision version 1.0 devices. It is optional for GigE Vision 1.1 
/// (and newer) compliant devices to implement Actions.
/// \par

#pragma pack ( push, 1 )
/// \brief   Standard GigE Vision GVCP Acknowledge telegram header structure.
typedef struct _J_tGVCP_ACK_HEADER
{
...
#pragma pack ( push, 1 )
/// \brief    Action-command acknowledge telegram structure.
typedef struct _J_tACTION_CMD_ACK
{
    J_tGVCP_ACK_HEADER  tAck;             ///< Standard GigE Vision GVCP Acknowledge telegram header
    uint32_t            uiAddress;        ///< IP address in transmission origin
} J_tACTION_CMD_ACK;
#pragma pack ( pop )

//******************************************************************************************************************
/// \brief   This function broadcasts a GigE Vision Action-command telegram on all network interfaces.
/// \par
///
/// \code
///    uint32_t	iNumOfAck;
...
///
///    // Is any acknowledges requested?
///    if(m_bReqAck && (m_AckCount!=0))
///    {
///        // Initialize the parameter for the J_Factory_SendActionCommand()
///        iNumOfAck = (uint32_t)m_AckCount;
///        // Allocate a buffer large enough to hold all the requested acknowledges
...
///        OutputMessage(_T("Failed to send Action Command 1!"));
///    else
///    {
///        if(m_bReqAck && (m_AckCount!=0))
///        {
///            if(iNumOfAck == 0)
///                OutputMessage(_T("No Ack!"));
///            else
///            {
///                CString sMessage;
...
///        }
///        else
///            OutputMessage(_T("Action Command 1 sent sucessfully!"));
///    }
///
///    // Free up the memory allocated for the acknowledges
///    if(pAckBuffer != NULL)
///        delete [] pAckBuffer;
/// \endcode
///
/// \param [in] hFac                  Handle to a valid factory object, obtained by \c J_Factory_Open()
/// \param [in] iDeviceKey            Device-key that authorize the action on the target devices.
/// \param [in] iGroupKey             Group-key that extend the range of separate device groups.
/// \param [in] iGroupMask            Group-mask that trigger action in multiple groups of devices separately.
/// \param [in,out] pAckBuffer        Pointer to a buffer to receive acknowledge information of Action-command. 
/// \param [in,out] piNumOfAck        The maximum number of acknowledgement information to be received (number of structures \c J_tACTION_CMD_ACK allocated and pointed to by \c pAckBuffer). This value will be updated with the actual number of received acknowledge telegrams.
/// \retval                           Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \sa \c J_Factory_Open()
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Factory_SendActionCommand(FACTORY_HANDLE hFac, uint32_t iDeviceKey, uint32_t iGroupKey, uint32_t iGroupMask, J_tACTION_CMD_ACK* pAckBuffer, uint32_t* piNumOfAck);

/// @}
/// \defgroup WRAP_IMAGE_PROCESSING   Image Processing functions
/// \ingroup WRAP_IMAGE
/// @{
/// \brief The Image Processing functions makes it possible for the user to apply a number of different image processing algorithms to an image buffer

/// \brief Image processing type used in \c J_Image_Processing() and \c J_Image_ProcessingEx()
typedef enum _J_IMAGE_PROCESS_TYPE
{
    ...
    LENS_DISTORTION_MAP_INIT,       //!< Lens distortion map initialization. Has to be used before LENS_DISTORTION and LENS_DISTORTION_MULTI in order to optimize speed
    LENS_DISTORTION_MAP_FREE,       //!< Lens distortion map free. Has to be called if LENS_DISTORTION_MAP_INIT has been used in order to free up resources
} J_IMAGE_PROCESSING;


//******************************************************************************************************************
/// \brief  This function processes the image by the function specified by the parameter.
///
/// \param [in] pImageInfo            Pointer to J_tIMAGE_INFO structure with information on the image data.
...
/// \par
/// \code
/// // C Sample that converts a raw image and performs Red-Compensation on the full image
/// BOOL RedCompensation(J_tIMAGE_INFO* ptRawImageInfo)
/// {
///   J_tIMAGE_INFO tCnvImageInfo;    // Image info structure
///   J_tCOLOR_COMPEN tColorCompen;   // Red-compensation parameter structure
///
///   // Allocate the buffer to hold converted the image
///   if (J_Image_Malloc(ptRawImageInfo, &tCnvImageInfo) != J_ST_SUCCESS)
///     return FALSE;
...
///     return FALSE;
///
///   tColorCompen.NumOfROI = 1;
///   tColorCompen.CompenParams[0].RectOfROI.left = 0;
///   tColorCompen.CompenParams[0].RectOfROI.right = ptRawImageInfo->iSizeX - 1;
///   tColorCompen.CompenParams[0].RectOfROI.top = 0;
///   tColorCompen.CompenParams[0].RectOfROI.bottom = ptRawImageInfo->iSizeY - 1;
///
///   // Threshold
///   tColorCompen.CompenParams[0].RThreshold = m_iRThresh;
///   tColorCompen.CompenParams[0].GThreshold = m_iGThresh;
///   tColorCompen.CompenParams[0].BThreshold = m_iBThresh;
///
///   // Compensation gain
///   tColorCompen.CompenParams[0].RGain = 4096 * m_iRGain / 100;
///   tColorCompen.CompenParams[0].GGain = 4096 * m_iGGain / 100;
///   tColorCompen.CompenParams[0].BGain = 4096 * m_iBGain / 100;
///
///   // Perform the Red-Compensation
///   J_Image_Processing(&tCnvImageInfo, RED_COMPENSATION, reinterpret_cast<void *>(&tColorCompen));
///
///   // Show the image
///   J_Image_ShowImage(m_hView, &tCnvImageInfo);
///
///   // Free up the image buffer
///   if (J_Image_Free(&tCnvImageInfo) != J_ST_SUCCESS)
///     return FALSE;
...
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_Processing(J_tIMAGE_INFO* pImageInfo, J_IMAGE_PROCESSING iProcessFunction, void * pParameters);

//******************************************************************************************************************
/// \brief  This function processes the image by the function specified by the parameter.
///
/// \param [in] pInputImageInfo       Pointer to J_tIMAGE_INFO structure with information on the image data.
/// \param [in] pOutputImageInfo      Pointer to J_tIMAGE_INFO structure with information on the image data.
/// \param [in] iProcessFunction      Type of image processing.
/// \param [in] pParameters           Pointer to parameters.
/// \retval                           Status code defined in the J_STATUS_CODES. If the function succeeds, returns J_ST_SUCCESS.
/// \par
/// \code
/// // C Sample that perform lens distortion correction on the full image
/// BOOL Undistort(J_tIMAGE_INFO* ptRawImageInfo)
/// {
///   J_tIMAGE_INFO tCnvImageInfo;        // Image info structure
///   J_tIMAGE_INFO tUndistortImageInfo;  // Image info structure
///   J_tUNDISTORT_PARAMS tUndistort;     // Lens Distortion parameter structure
///
///   // Allocate the buffer to hold converted the image
///   if (J_Image_Malloc(ptRawImageInfo, &tCnvImageInfo) != J_ST_SUCCESS)
///     return FALSE;
///
///   // Allocate the buffer to hold the undistorted image. It is same size as the converted image
///   if (J_Image_Malloc(ptRawImageInfo, &tUndistortImageInfo) != J_ST_SUCCESS)
///     return FALSE;
///
///   // Convert the raw image to image format
///   if (J_Image_FromRawToImage(ptRawImageInfo, &tCnvImageInfo) != J_ST_SUCCESS)
...
///   tUndistort.dOpticalCenterX = m_OpticalCenterX; // Typically close to the image center (ptRawImageInfo->iSizeX/2)
///   tUndistort.dOpticalCenterY = m_OpticalCenterY; // Typically close to the image center (ptRawImageInfo->iSizeY/2)
///   tUndistort.dFocalLengthX = m_FocalLengthX;     // Typically the image width (ptRawImageInfo->iSizeX)
///   tUndistort.dFocalLengthY = m_FocalLengthY;     // Typically the image width (ptRawImageInfo->iSizeX)
///   tUndistort.dCoeffK1 = m_CoeffK1;               // Depending on lens (around -0.33246 for a std. 4mm lens)
///   tUndistort.dCoeffK2 = m_CoeffK2;               // Depending on lens (around  0.01839 for a std. 4mm lens)
///   tUndistort.dCoeffK3 = m_CoeffK3;               // Depending on lens (around  0.0 unless it is a fish-eye lens)
///   tUndistort.dCoeffP1 = m_CoeffP1;               // Depending on lens (around -0.00020 for a std. 4mm lens)
///   tUndistort.dCoeffP2 = m_CoeffP2;               // Depending on lens (around  0.00022 for a std. 4mm lens)
///
///   // Perform the Red-Compensation (and since it works on the image directly then the output is empty)
///   J_Image_ProcessingEx(&tCnvImageInfo, &tUndistortImageInfo, LENS_DISTORTION, reinterpret_cast<void *>(&tUndistort));
///
///   // Show the image
///   J_Image_ShowImage(m_hView, &tUndistortImageInfo);
///
///   // Free up the conversion image buffers
///   if (J_Image_Free(&tCnvImageInfo) != J_ST_SUCCESS)
///     return FALSE;
/// 
///   if (J_Image_Free(&tUndistortImageInfo) != J_ST_SUCCESS)
///     return FALSE;
/// 
///   return TRUE;
/// }
/// \endcode
EXTERN_C GCFC_IMPORT_EXPORT J_STATUS_TYPE __cdecl J_Image_ProcessingEx(J_tIMAGE_INFO* pInputImageInfo, J_tIMAGE_INFO* pOutputImageInfo, J_IMAGE_PROCESSING iProcessFunction, void * pParameters);

/// @}
/// \defgroup WRAP_IMAGE_PROCESSING_COLOR_COMPENSATION   Color Compensation Image Processing algorithm
/// \ingroup WRAP_IMAGE_PROCESSING
/// @{
/// \brief The Color Compensation Image Processing algorithms (Red- and Green-compensation) makes it possible artificially enhance the red and/or green color near saturation.
/// This is typically used in ITS applications where the cameras are looking at traffic lights at night. This will help remove dis-coloration caused by saturation of the colors.
/// The color compensation can be applied in up to 10 seperate Regions-of-Interests specified in the \c RectOfROI parameter of the \c J_tCOLOR_COMPEN_PARAMS structure.

/// \brief Struct containing parameters for color compensation for each ROI.
...
/// \brief Struct containing parameters for color compensation. This is used for both Red-compensation and Green-compensation
typedef struct _J_tCOLOR_COMPEN
{
    uint32_t                NumOfROI;           ///< Number of ROI for color compensation. This has to be a value between 1 and 10
    J_tCOLOR_COMPEN_PARAMS  CompenParams[10];   ///< Array of structures of parameters (one structure per ROI).
} J_tCOLOR_COMPEN;
/// @}
/// @}
/// \defgroup WRAP_IMAGE_PROCESSING_LENS_DISTORTION   Lens Distortion Correction Image Processing algorithm
/// \ingroup WRAP_IMAGE_PROCESSING
/// @{
/// \brief The Lens Distortion Image Processing algorithms makes it possible to correct for Radial Distortion as well as Tangential Distortion caused by the lens.
/// This is typically used to correct for distortion caused by using wide-angle lenses.
/// The lens distortion correction parameters are specified in the \c J_tUNDISTORT_PARAMS structure and it is only to do lens distortion correction using \c J_Image_ProcessingEx() since it requires an output image!

/// \brief Interpolation type used in \c J_tUNDISTORT_PARAMS
typedef enum _J_INTERPOLATION_TYPE
{
   INTERPOLATION_NONE = 0,      //!< No interpolation. Closest pixel is used instead
   INTERPOLATION_BILINEAR = 1,  //!< Bi-linear interpolation is used
} J_INTERPOLATION_TYPE;


/// \brief Struct containing parameters for lens distortion correction.
typedef struct _J_tUNDISTORT_PARAMS
{
    ...
    double dCoeffK1;            ///< Radial Distortion coefficient k1 for r^2 in Taylor series expansion around r=0: Xcorr = X(1+k1*r^2+k2*r^4+k3*r^6),  Ycorr = Y(1+k1*r^2+k2*r^4+k3*r^6)
    ...
    int    iInterpolationType;  ///< Type of interpolation used for the lens distortion. Specified as one of the \c J_INTERPOLATION_TYPE enumeration values
    void*  pMapCache;           ///< Internal structure used for speedoptimization. Has to be set to 0 before initializing the map using 
} J_tUNDISTORT_PARAMS; 
/// @}
/// @}
#endif
