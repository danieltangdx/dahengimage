...
#if !defined(ParamDefine_h_)
#define ParamDefine_h_
...
typedef struct _ImageInformationStruct
{
...
{
	int iStationSN{0};			//工位号
	int iCircleSN{0};			//采集号
	int iImageSN{0};			//图像号
	int iIsUsing{0};			//是否向内存写入图像数据,0为未写，1为正在写，2为写入完毕
	int iIsDetecting{0};		//是否正在检测
	BOOL bHaveError{0};		//是否有错
	BOOL bIsCheck{0};			//是否正在检测
	BYTE* pAddr{NULL};			//内存地址
	CString strErrorInfo{TEXT("")};	//报错信息
}s_ElementInfo;
