...
BOOL CBottleCapView::InitWindowRect()
{
	...
	{
		SetDlgItemText(IDC_USER_ADM,"用户管理");
	}
	else
	{
		SetDlgItemText(IDC_USER_ADM,"密码修改");
	}
...
// 	m_Tab1.SetCurSel(0);
	...
	m_SpeedLabel.Display("检测速度:","",0,FALSE);
	...
	m_UserLabel.Display("用户名称:","",0,FALSE);
...
//				bRet = g_pFrame->OP_Directory(str,NULL,FO_DELETE,FOF_NOCONFIRMATION | FOF_SIMPLEPROGRESS |FOF_NOERRORUI);
			...
			for(int i=0;i<CAMERA_COUNT;i++)
			{
				...
				sIn.iReserve3= g_pFrame->m_iSaveImgObj;			//iReserve3为0算法存图，1为多对象存图
...
void CBottleCapView::OnIocard() 
{
	...
	{
		AfxMessageBox("接口卡无法打开，请确定有无接口卡!");
	}
}
		...
		g_pFrame->m_bSaveCameraImage[Camera3])
	{
		if(IDYES==AfxMessageBox("正在保存图像，是否停止保存图像!",MB_YESNO))
		{
			for(int i=0;i<CAMERA_COUNT;i++)
			{
				...
				sIn.iOperSig = 301;
				sIn.iReserve1 = m_iSaveImageType;
				sIn.iReserve2 = 0;//**[10/23/2017 ConanSteve]**:  0不存图
