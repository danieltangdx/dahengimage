...
2016-11-11
邵建超：（1）保存图像类型中增加：3-有瓶 4-无瓶 两种存图类型
（2）按指定错误类型保存，这种方式需要算法判断错误类型是否为需要保存的类型。
例如：保存类型为52，即客户想保存瓶底裂纹缺陷品，则各个算法判断返回值是否是52，如果是则保存
...
namespace LampInsp {
	...
	typedef struct _DetectInParam
	{
		...
		int	iRejectTest;	// 0表示正常检测 1表示“踢废测试 全NG” 2表示“踢费测试 全OK”
							...
							/////2016.05.25新增存图/////////////////////////////////////////////////////////////////////
		int iSaveType;		// 存图类型 0-所有（有无都存） 1-坏品  2-好品，3-有瓶 4-无瓶 -1-异常
		char* cSavePath;	// 前端传入存图路径指定到文件夹，比如R:\SaveImages\AllImages\2016-10-14 09-38. 算法根据iCricleIndex和iSaveType以及iCheckSN保存图像
