...
public:
	...
	bool AddElem( LPCTSTR szName, LPCTSTR szData=_T(""), int nFlags=0 ) { return x_AddElem(szName,szData,nFlags); };
	bool InsertElem( LPCTSTR szName, LPCTSTR szData=_T(""), int nFlags=0 ) { return x_AddElem(szName,szData,nFlags|MNF_INSERT); };
	bool AddChildElem( LPCTSTR szName, LPCTSTR szData=_T(""), int nFlags=0 ) { return x_AddElem(szName,szData,nFlags|MNF_CHILD); };
	bool InsertChildElem( LPCTSTR szName, LPCTSTR szData=_T(""), int nFlags=0 ) { return x_AddElem(szName,szData,nFlags|MNF_INSERT|MNF_CHILD); };

//yqs添加
	bool AddElem( LPCTSTR szName, DWORD dwValue, int nFlags=0 ) { return x_AddElem(szName,dwValue,nFlags); };
	bool InsertElem( LPCTSTR szName, DWORD dwValue, int nFlags=0 ) { return x_AddElem(szName,dwValue,nFlags|MNF_INSERT); };
	bool AddChildElem( LPCTSTR szName, DWORD dwValue, int nFlags=0 ) { return x_AddElem(szName,dwValue,nFlags|MNF_CHILD); };
	bool InsertChildElem( LPCTSTR szName, DWORD dwValue, int nFlags=0 ) { return x_AddElem(szName,dwValue,nFlags|MNF_INSERT|MNF_CHILD); };
//yqs添加
...
//yqs添加
	bool AddAttrib( LPCTSTR szAttrib, DWORD dwValue ) { return x_SetAttrib(m_iPos,szAttrib,dwValue); };
	bool AddChildAttrib( LPCTSTR szAttrib, DWORD dwValue ) { return x_SetAttrib(m_iPosChild,szAttrib,dwValue); };
	...
	// yqs增加
	void GetDataEx(int& nData){nData = _ttoi(GetData());};
	void GetDataEx(double& dbData){dbData = _ttof(GetData());};
	void GetDataEx(float& fData){fData = (float)_ttof(GetData());};
	void GetDataEx(DWORD& dwData){_tscanf_s(GetData(),_T("%X"),&dwData);};

	void GetAttribEx(LPCTSTR szAttrib, int& nData){nData = _ttoi(GetAttrib(szAttrib));};
	void GetAttribEx(LPCTSTR szAttrib, DWORD& dwData){ _tscanf_s(GetAttrib(szAttrib),_T("%X"),&dwData);};
	void GetAttribEx(LPCTSTR szAttrib, double& dbData){dbData = _ttof(GetAttrib(szAttrib));};
	void GetAttribEx(LPCTSTR szAttrib, float& fData){fData = (float)_ttof(GetAttrib(szAttrib));};
	void GetAttribEx(LPCTSTR szAttrib, short& nData){nData = (short)_ttoi(GetAttrib(szAttrib));};
	
	void GetChildAttribEx(LPCTSTR szAttrib, int& nData){nData = _ttoi(GetChildAttrib(szAttrib));};
	void GetChildAttribEx(LPCTSTR szAttrib, DWORD& dwData){ _tscanf_s(GetAttrib(szAttrib),_T("%X"),&dwData);};
	void GetChildAttribEx(LPCTSTR szAttrib, double& dbData){dbData = _ttof(GetChildAttrib(szAttrib));};
	void GetChildAttribEx(LPCTSTR szAttrib, float& fData){fData = (float)_ttof(GetChildAttrib(szAttrib));};
	void GetChildAttribEx(LPCTSTR szAttrib, short& nData){nData = (short)_ttoi(GetChildAttrib(szAttrib));};
	//yqs添加
	...
	//yqs添加
	bool SetAttrib(LPCTSTR szAttrib, DWORD dwValue) { return x_SetAttrib(m_iPos, szAttrib, dwValue); };
	bool SetChildAttrib(LPCTSTR szAttrib, DWORD dwValue) { return x_SetAttrib(m_iPosChild, szAttrib, dwValue); };
	...
	//yqs添加
	bool SetData(DWORD dwValue) { return x_SetData(m_iPos, dwValue); };
	bool SetChildData(DWORD dwValue) { return x_SetData(m_iPosChild, dwValue); };

	bool SetElemContent(LPCTSTR szContent) { return x_SetElemContent(szContent); };


	CString MemToStr(TCHAR* pBuf, int nLen);	
	int	StrToMem(TCHAR* pBuf, int nBufLen, CString strBlock);//内存块字符化,返回数据长度(字节)nBufLen内存区大小
...
protected:
	...
	struct PosArray
	{
		...
		void Release() { for (int n=0;n<SegsUsed();++n) delete[] (TCHAR*)pSegs[n]; if (pSegs) delete[] (TCHAR*)pSegs; };
...
//yqs添加
	...
	bool x_AddElem( LPCTSTR szName, DWORD dwValue, int nFlags );
...
//yqs添加
	...
	bool x_SetAttrib( int iPos, LPCTSTR szAttrib, DWORD dwValue);
...
//yqs添加
	...
	bool x_SetData( int iPos, DWORD dwValue );
