...
s_Status CDataClass::Init(s_CheckInitParam sInitParam)
{
	...
	if (e_CallType_Demo != m_iCallType)
	{
		...
		/************************************************************************/
		m_ppCopyWindowParaCacheQueue = make_unique<unique_ptr<CAlgCopyWindow[]>[]>(m_iCheckGroupNum);
		m_ppCopyWindowParaAlgObjQueue = make_unique<unique_ptr<CAlgCopyWindow[]>[]>(m_iCheckGroupNum);
		for (i = 0; i < m_iCheckGroupNum; i++)
		{
			// 创建触发检测线程内核事件
...
// 
			...
			//m_pIfNeedSaveImg[i] = shared_ptr<bool>(new bool[m_iGroupCheckQueueNum[i]], default_delete<bool[]>());

			m_ppCopyWindowParaCacheQueue[i] = make_unique<CAlgCopyWindow[]>(m_iGroupCheckQueueNum[i]);
			m_ppCopyWindowParaAlgObjQueue[i] = make_unique<CAlgCopyWindow[]>(m_pnObjectNumber[i]);
...
s_Status CDataClass::Set(s_AlgModelInput sAlgModelInput)
{
	...
	case e_AlgModelInputOper_LoadModel:
	{
		...
		for (i = 0; i < m_pnObjectNumber[iCheckSN]; ++i)
		{
			...
			CSetAlgObject modelObject;
			modelObject.SetAddrOfModelObj((void*)&m_iIdleJob[0]);
			modelObject.SetAlgObjIndex(i);
			//modelObject.SetLoadModelCtrlPara(e_LoadModelCtrlPara_ReadModel);
			extenInfo.push_back(&modelObject);
	...
	case e_AlgModelInputOper_UseExtenInfoFunc:
	{
		...
		CSetObjStatusWnd* pSetObjStatusWnd{ NULL };
		pExtenInfo = (CExtenInfo*)sAlgModelInput.iReserve4;
		if (NULL == pExtenInfo)
			return sError;
		pSetObjStatusWnd = (CSetObjStatusWnd*)pExtenInfo->GetFuncAddr(Sys2Obj_SetObjStatus);
		if (NULL == pSetObjStatusWnd)
			return sError;
		//if (m_bShowObjRunInfoWnd != pSetObjStatusWnd->GetIsShow())
		{
			m_bShowObjRunInfoWnd = pSetObjStatusWnd->GetIsShow();
			if (m_bShowObjRunInfoWnd)
			{
				m_pDlgRunInfo->ShowWindowEx(SW_SHOW);
			}
...
BOOL CDataClass::InitObjects(CString strInitFile, int iCallType)//测试动态库是否可以调用
{
	...
	m_bEnableExtenInfo = m_iniAlgCfg.GetBool(_T("SystemConfiguration"), _T("EnableExtenInfo"), false, _T("是否使用多对象扩展信息结构体"));
	m_bEnableOvertimeCallback = m_iniAlgCfg.GetBool(_T("Systemconfiguration"), _T("EnableOvertimeCallback"), true, _T("是否使用超时回调"));
	m_iSysDetectMode = m_iniAlgCfg.GetInt(_T("SystemConfiguration"), _T("SystemDetectMode"), e_SysDetectMode_Multi, _T("系统检测模式,1为多对象收图检测。0为系统收图"));
	m_iObjDetectMode = m_iniAlgCfg.GetInt(_T("SystemConfiguration"), _T("ObjectDetectMode"), e_ObjDetectMode_1, _T("0为1次Detect，1为2次Detect"));
	m_bShowObjRunInfoWnd = m_iniAlgCfg.GetBool(_T("SystemConfiguration"), _T("EnableObjectRunInfoWindow"), false, _T("是否打开窗口运行信息"));
	m_bEnableXCGUI = m_iniAlgCfg.GetBool(_T("SystemConfiguration"), _T("EnableXCGUI"), false, _T("是否使用XCGUI组件"));
	m_iSingleTest = m_iniAlgCfg.GetInt(_T("SystemConfiguration"), _T("SingleTest"), -1, _T("进行单路测试，-1时，正常运行"));
	m_bEnableInitObjectAndCheck = m_iniAlgCfg.GetBool(_T("SystemConfiguration"), _T("EnableInitMultiObjectAndCheck"), true, _T("是否执行多对象的初始化和算法检测工作"));
	m_bEnableAlgInit = m_iniAlgCfg.GetBool(_T("SystemConfiguration"), _T("EnableAlgInit"), true, _T("是否执行算法的初始化工作"));
	/************************************************************************/
	...
	for (i = 0; i < m_iCheckGroupNum; ++i)
	{
		...
		sVal = m_iniAlgCfg.GetString(strSegment, strDllKeyName, sVal, _T("NULL"));
	...
	m_iCheckThreadNum = m_iniAlgCfg.GetInt(_T("Basic Parameters"), _T("CheckThreadNum"), 10, _T("检测线程个数"));

	// 读取相机个数

	m_iCameraNum = m_iniAlgCfg.GetInt(_T("Basic Parameters"), _T("CameraNum"), 1, _T("相机个数"));

	// 读取剔废队列个数
	strSegment.Format(_T("Basic Parameters"));
	m_iKickQueueNum = m_iniAlgCfg.GetInt(_T("Basic Parameters"), _T("KickQueueNum"), 3);
	...
	for (i = 0; i < m_iCameraNum; i++)
	{
		...
		strAlg.Format(_T("Height"));
		GetPrivateProfileString(strSegment, strAlg, _T("954"), sVal.GetBuffer(80), 80, strInitFile);
		sVal.ReleaseBuffer();
		m_iImageHeight[i] = _ttoi(sVal.GetString());
		...
		m_iImageByte[i] = m_iniAlgCfg.GetInt(strSegment, strAlg, 1, _T("每像素字节数"));
		...
		m_iCheckStatusSize[i] = m_iniAlgCfg.GetInt(strSegment, strAlg, 200, _T("算法检测状态结构体中的void* pParam大小（字节为单位）"));
	...
	for (i = 0; i < m_iCheckGroupNum; i++)
	{
		...
		m_iGroupCheckQueueNum[i] = m_iniAlgCfg.GetInt(strSegment, _T("ElementNum"), 2, _T("每个检测组缓冲队列的长度"));
	}
	...
	for (i = 0; i < m_iKickQueueNum; i++)
	{
		...
		m_iTimeType[i] = m_iniAlgCfg.GetInt(strSegment, strAlg, 0, _T("剔废队列1定时基准 0为时间，单位为毫秒；1为帧/行，计时单位为帧个数/编码器行频"));

		strAlg.Format(_T("TimeOut"));
		CString sAnnotation = 0 == m_iTimeType[i] ? _T("毫秒") : _T("帧/行");
		m_iTimeOut[i] = m_iniAlgCfg.GetInt(strSegment, strAlg, 120);
	}
	...
	m_iCallInterval = m_iniAlgCfg.GetInt(strSegment, strAlg, 20, _T("同一相机同一图像号最小触发间隔"));

	strSegment.Format(_T("Other Parameters"));
	strAlg.Format(_T("MinIntervalOfDiffPeriod"));
	m_iPeriodInterval = m_iniAlgCfg.GetInt(strSegment, strAlg, 30);

	strSegment.Format(_T("Other Parameters"));
	strAlg.Format(_T("bWriteLog"));
	m_bWriteLog = m_iniAlgCfg.GetBool(strSegment, strAlg, false);

	strSegment.Format(_T("Other Parameters"));
	strAlg.Format(_T("MaxLogNum"));
	m_iMaxLogLineNum = m_iniAlgCfg.GetInt(strSegment, strAlg, 10000);
	...
	m_iDefaultCheckRslt = m_iniAlgCfg.GetInt(_T("SystemConfiguration"), _T("DefaultCheckResult"), -3, _T("剔废表默认检测结果"));
	m_iLostImageRslt = m_iniAlgCfg.GetInt(_T("SystemConfiguration"), _T("DefaultLostImageResult"), -4, _T("剔废表在多对象主动丢图时写的检测结果"));
...
s_Status CDataClass::Detect(s_CheckInParam sCheckInParam)
{
	...
	if (!m_bEnableInitObjectAndCheck||!m_bEnableAlgInit)
		return sError;
	if (-1 != m_iSingleTest)
	{
		...
		if (TRUE == JudgeNewCircle(iGroupSN, iCheckSN, iImageSN, iCircleSN))
		{
			...
			if (true == bBusy)
			{
				...
				if (MAX_IMAGE_COUNT != iMinCircleSN)
				{
					...
					m_ppCopyWindowParaCacheQueue[iGroupSN][iCurrIndex].SetTickOfPushQueue(liTime.QuadPart);
...
/************************************************************************/
	...
	for (i = 0; i < m_iCheckGroupNum; ++i)
	{
		...
		if (hDll)
		{
			...
			if (destroyobj&&m_bEnableAlgInit)
			{
...
DWORD CDataClass::FnThreadSaveImgFunc()
{
	while (1)
	{
		...
		while (!m_queImgsWaitForSave.empty())
		{
			...
			CMultiObjSaveImgPara* pMultiObjSaveImgPara = (CMultiObjSaveImgPara*)(pExtenInfo->GetFuncAddr(Sys2Obj_SaveImgPara));
			
			CString sSaveImgFolderPath{ _T("") };
#ifdef UNICODE
			wstring wsSaveImgFolderPath = CStringConvert::ANSI_UCS2(pMultiObjSaveImgPara->GetAddrOfSaveImgFolderPath());
			sSaveImgFolderPath.Format(_T("%s"), wsSaveImgFolderPath.c_str());
#else
			sSaveImgFolderPath.Format(_T("%s"), pMultiObjSaveImgPara->GetAddrOfSaveImgFolderPath());
#endif
			sSaveImgFolderPath.Format(_T("%s"), pMultiObjSaveImgPara->GetAddrOfSaveImgFolderPath());
			/** Check Disk free space              [6/12/2017 ConanSteve]**/
			{
					...
					/*       存图                [6/12/2017 ConanSteve]                         */
				{
					...
					m_srwlImgQue[iGroupSN].Lock();
					for (int i = 0; i <pMultiObjSaveImgPara->GetImgNum(); ++i)
					{
						sSaveImgFilePath.Format(_T("\\%02d.bmp"), i + 1);
						sSaveImgFilePath = sSaveImgFolderPath + sSaveImgFilePath;
						unsigned char* pAddr = (unsigned char*)(sImgInfoWaitForSave.sCheckInParam.pImageData + i*(pMultiObjSaveImgPara->GetImgWidth())*(pMultiObjSaveImgPara->GetImgHeight())*(pMultiObjSaveImgPara->GetImgChannel()));
						SaveBmp(sSaveImgFilePath, pMultiObjSaveImgPara->GetImgWidth(), pMultiObjSaveImgPara->GetImgHeight(), pMultiObjSaveImgPara->GetImgChannel(), pAddr);
					}
					m_srwlImgQue[iGroupSN].Unlock();
					/************************************************************************/
...
void CDataClass::CheckEx2(int iIndex, int iThreadID, s_CheckInParam sCheckInParam)
{
	...
	else
	{
		...
		m_ppCopyWindowParaAlgObjQueue[iGroupSN][iIdleObjIndex] = m_ppCopyWindowParaCacheQueue[iGroupSN][iIndex];
		LARGE_INTEGER liEndTime,liFreq;
		QueryPerformanceFrequency(&liFreq);
		QueryPerformanceCounter(&liEndTime);
		m_ppCopyWindowParaAlgObjQueue[iGroupSN][iIdleObjIndex].SetTimeOfWait((liEndTime.QuadPart - m_ppCopyWindowParaAlgObjQueue[iGroupSN][iIdleObjIndex].GetTickOfPushQueue()) * 1000 / liFreq.QuadPart);
		/*************************************<检测>*************************************/
		{
			CExtenInfo extenInfo;
			...
			CAlgDetectPara algDetectPara;
			extenInfo.push_back(&algDetectPara);
			extenInfo.push_back(&m_ppCopyWindowParaAlgObjQueue[iGroupSN][iIdleObjIndex]);
			if (e_ObjDetectMode_1 == m_iObjDetectMode)
			{
				...
				algDetectPara.SetDetectTimes(0);
			...
			else if (e_ObjDetectMode_2 == m_iObjDetectMode)
			{
				...
				algDetectPara.SetDetectTimes(1);
				...
				*spCheckStatusParam = pInsp->Detect(sCheckInParam, spCheckOutParam);
				...
				algDetectPara.SetDetectTimes(2);
		...
		m_ppCopyWindowParaAlgObjQueue[iGroupSN][iIdleObjIndex].SetTimeOfFirstDetect(iElapse1);
		m_ppCopyWindowParaAlgObjQueue[iGroupSN][iIdleObjIndex].SetTimeOfSecondDetect(iElapse2);
		...
		sLogInfo.Format(_T("[检测组号:%2d] [线程ID:%2d] [图像号:%8d] [缓存队列索引:%2d] [是否存图:%2s] [是否画图:%2s] [1次Detect时间:%4d] [2次Detect时间:%4d] [算法检测时间:%4d] [数据等待时间:%8d]"),
			...
			iElapse1, iElapse2, iElapse1+ iElapse2, m_ppCopyWindowParaAlgObjQueue[iGroupSN][iIdleObjIndex].GetTimeOfWait());
			...
			m_iGroupSaveImageCount[iGroupSN] < m_iGroupSaveImageCount_SET[iGroupSN])	//保存张数未满
		{
			...
			CMultiObjSaveImgPara *pMultiObjSaveImgPara{ NULL };
			//**[7/14/2017 ConanSteve]**:  使用了扩展结构体
			if (0 != sCheckInParam.iReserve4)
			{
				pExtenPara = (CExtenInfo*)(sCheckInParam.iReserve4);
				pMultiObjSaveImgPara = (CMultiObjSaveImgPara *)pExtenPara->GetFuncAddr(Sys2Obj_SaveImgPara);
				//pMultiObjSaveImgPara = (CMultiObjSaveImgPara *)pExtenPara->GetFuncAddr(typeid(CMultiObjSaveImgPara));
				m_bIsCircle = pMultiObjSaveImgPara->GetIsCirCleSaveImg();
			}	
			if (NULL != pMultiObjSaveImgPara)
			{
				...
				else
				{
					...
					while (ulFreeSpace <= m_iSizeOfOneArrImgs || ulFreeSpace <= 50 * 1024 * 1024)
					{
						...
						if (!m_queSaveImgHistory.empty())
						{
							...
							for (auto itera = vec_sSaveImgPaths.begin(); itera != vec_sSaveImgPaths.end(); ++itera)
							{
								CExFn::DeleteDirectory((*itera).GetBuffer());
							}
			...
			//**[7/14/2017 ConanSteve]**:  算法存图
			else //if (e_SaveImgObj_Alg == m_iSaveImgObj)
			{
				if (bExecuteSaveImgOper)
				{
					/*****************************************************************************************/
					//**[7/14/2017 ConanSteve]**:  存图
					...
					if (NULL != pMultiObjSaveImgPara)
					{
						...
						CString sAlgConfigPath = m_strAppDir + _T("/Config/AlgConfig.ini");
						bool bEnableSequentialCheck = m_iniAlgCfg.GetBool(_T("Demo Controls"), _T("EnableSequentialCheck"), 0, sAlgConfigPath);
						CString sSequentialCheckFolderFilter{ _T("") };
						if (bEnableSequentialCheck)
						{
							sSequentialCheckFolderFilter = m_iniAlgCfg.GetString(_T("Demo Controls"), _T("SequentialCheckFolderFilter"), sSequentialCheckFolderFilter);
						}
						sSaveImgFolderPath = (CString)pMultiObjSaveImgPara->GetAddrOfSaveImgFolderPath();
						sImgSNName.Format(_T("\\%s%08d"), bEnableSequentialCheck ? sSequentialCheckFolderFilter : _T("Product_"), iImgSN);
						sSaveImgFolderPath += sImgSNName;
						if (m_arr_iCamSmallImgNum[iGroupSN] == 1)
							sSaveImgFolderPath += _T(".bmp");
						//CreateDirectory(sSaveImgFolderPath, NULL);
						/*****************************************************************************************/
					...
					char buf[512]{ 0 };
					memcpy_s(buf, 512, sSaveImgFolderPath.GetBuffer(), sSaveImgFolderPath.GetLength());
					sIn.iReserve3 = (int)buf;
					CExtenInfo extenInfoObj2Alg;
					CAlgSaveImgPara algSaveImgPara;
					extenInfoObj2Alg.push_back(&algSaveImgPara);
					algSaveImgPara.SetImgSN(iImgSN);
					algSaveImgPara.SetImgSavedCnt(m_iSaveImageNum[iGroupSN]);
					algSaveImgPara.SetSaveImgFolderPath(sSaveImgFolderPath.GetBuffer(), sSaveImgFolderPath.GetLength());
					TCHAR *lpsz = algSaveImgPara.GetAddrOfSaveImgFolderPath();
					sIn.iReserve4 = (int)&extenInfoObj2Alg;
