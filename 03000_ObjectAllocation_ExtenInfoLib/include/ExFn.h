...
{
	VarType varType{VarType::__no};//**[9/14/2017 ConanSteve]**:  变量类型
	CString sAppName{ _T("") };
	CString sKeyName{ _T("") };
	union {
		bool *pbBool;
...
public:
	...
	int& GetIntEx(LPCTSTR lpszAppName, LPCTSTR lpszKeyName, int& iKeyValue, int iDefaultValue = -1, LPCTSTR lpszAnnotation = NULL);//**[9/14/2017 ConanSteve]**:  绑定变量，当类析构时，会自动写入本次调用绑定的变量
	bool WriteInt(LPCTSTR lpszAppName, LPCTSTR lpszKeyName, int iKeyValue, LPCTSTR lpszAnnotation = NULL);
	bool GetBool(LPCTSTR lpszAppName, LPCTSTR lpszKeyName, bool bDefaultValue =false, LPCTSTR lpszAnnotation = NULL);
	bool GetBoolEx(LPCTSTR lpszAppName, LPCTSTR lpszKeyName, bool& bRetKeyValue, bool bDefaultValue = false, LPCTSTR lpszAnnotation = NULL);
	bool WriteBool(LPCTSTR lpszAppName, LPCTSTR lpszKeyName, bool bKeyValue, LPCTSTR lpszAnnotation = NULL);
...
private:
	...
	int m_iCurLine{ 0 };//**[8/30/2017 ConanSteve]**:  当前记录行数
	int m_iMaxLine{ 10000 };//**[8/30/2017 ConanSteve]**:  最大可记录行数
	unsigned __int64 m_iLastTick{ 0 };//**[9/5/2017 ConanSteve]**:  上一次时钟计数
	unsigned __int64 m_iFrequency{ 1 };//**[9/5/2017 ConanSteve]**:  计算机CPU频率
