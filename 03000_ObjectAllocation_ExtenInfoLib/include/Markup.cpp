...
void CMarkup::operator=( const CMarkup& markup )
{
	...
	if ( m_aPos.nSegs )
	{
		m_aPos.pSegs = (ElemPos**)(new TCHAR[m_aPos.nSegs*sizeof(TCHAR*)]);
		...
		for ( int nSeg=0; nSeg < m_aPos.nSegs; ++nSeg )
		{
			...
			m_aPos.pSegs[nSeg] = (ElemPos*)(new TCHAR[nSegSize*sizeof(ElemPos)]);
...
#if defined(_UNICODE) // convert file to wide TCHAR
	int nWideLen = 0;
	if ( nFileByteLen )
	{
...
CString CMarkup::UnescapeText(LPCTSTR szText, int nTextLength /*=-1*/ )
{
	...
	// ampersand escape codes replaced with special characters e.g. convert "6&gt;7_T(" to ")6>7"
...
bool CMarkup::x_AllocPosArray( int nNewSize /*=0*/ )
{
	...
	if ( m_aPos.GetSize() < nNewSize )
	{
		...
		if ( m_aPos.nSegs <= nNewSeg )
		{
			...
			TCHAR* pNewSegments = new TCHAR[nNewSegments*sizeof(TCHAR*)];
			if ( m_aPos.SegsUsed() )
				memcpy( pNewSegments, m_aPos.pSegs, m_aPos.SegsUsed()*sizeof(TCHAR*) );
			if ( m_aPos.pSegs )
				delete[] (TCHAR*)m_aPos.pSegs;
		...
		{
			TCHAR* pNewFirstSeg = new TCHAR[ nFullSegSize * sizeof(ElemPos) ];
			if ( nSegSize )
			{
				// Reallocate
				memcpy( pNewFirstSeg, m_aPos.pSegs[nSeg], nSegSize * sizeof(ElemPos) );
				delete[] (TCHAR*)m_aPos.pSegs[nSeg];
			}
			m_aPos.pSegs[nSeg] = (ElemPos*)pNewFirstSeg;
		}

		// New segment
		TCHAR* pNewSeg = new TCHAR[ nNewSegSize * sizeof(ElemPos) ];
...
{
	// Starting at nChar, find a non-whitespace TCHAR
	// return false if no non-whitespace before end of document, nChar points to end
	// otherwise return true and nChar points to non-whitespace TCHAR
...
bool CMarkup::x_FindName( CMarkup::TokenPos& token )
{
	...
	// Go until special TCHAR or whitespace
	token.nL = nChar;
	while ( szDoc[nChar] && ! _tcschr(_T(" \t\n\r<>=\\/?!"),szDoc[nChar]) )
		nChar += (int)_tclen(&szDoc[nChar]);

	// Adjust end position if it is one special TCHAR
	if ( nChar == token.nL )
		++nChar; // it is a special TCHAR
	token.nR = nChar - 1;

	// nNext points to one past last TCHAR of token
	token.nNext = nChar;
	return true;
}
...
int CMarkup::x_ParseNode( CMarkup::TokenPos& token, CMarkup::NodePos& node )
{
	...
	// Upon return token.nNext points to the TCHAR after the node or tag
	// 
	// <!--...--> comment
	// <!DOCTYPE ...> dtd
...
bool CMarkup::x_SetAttrib( int iPos, LPCTSTR szAttrib, DWORD dwValue )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%X"), dwValue );
	return x_SetAttrib( iPos, szAttrib, szVal );
}
...
bool CMarkup::x_SetData( int iPos, DWORD dwValue )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%XH"), dwValue );
	return x_SetData( iPos, szVal, 0 );
}
...
bool CMarkup::x_AddElem( LPCTSTR szName, DWORD dwValue, int nFlags )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%XH"), dwValue );
...
CString CMarkup::MemToStr(TCHAR* pBuf, int nLen)
{
	...
	{
		strTmp.Format(_T("%02X"), pBuf[i]&0x0ff);
		strRet += strTmp;
	}
	return strRet;
}
int	CMarkup::StrToMem(TCHAR* pBuf, int nBufLen, CString strBlock)
{
	if(pBuf == NULL)
	{
		return 0;
	...
	for(int i = 0 ; i < nLen; i++)
	{
		_tscanf_s(strBlock.Mid(i*2,2),_T("%X"),&nData);
		pBuf[i] = nData & 0xff;
	}
	return nLen;
}
